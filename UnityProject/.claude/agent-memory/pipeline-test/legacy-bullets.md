---
name: legacy-bullets
description: pipeline-test 历史经验整体归档(从 pipeline/memory/test.md 迁入,后续触发时按条独立化)
metadata:
  type: project
---

> 本文件是 2026-06-21 迁移期的整体归档:把 `pipeline/memory/test.md` 原 bullet list 一次性搬入,保留紧凑性。后续 test 收尾沉淀新经验时按结构化格式独立写(`feedback-*.md` / `project-*.md` 等),遇本文件内重复或过时条目可独立化或删除。
