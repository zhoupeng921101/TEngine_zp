...
| [editmode-reset-play-wired-static-hooks.md](editmode-reset-play-wired-static-hooks.md) | GameApp 在 Play 装配的静态生产钩子(BlockGameState.OnMergeStateReady 等,体内注入 ServerDeal / 切服务端权威)跨 EditMode 域存活,污染纯逻辑单测的本地权威基线,呈「fresh 域绿、Play 后红」flaky;在 ResetConfigCachesAttribute 每用例前后清零钩子(清单例不够),及 get_test_job 不可用时 execute_code in-process 对比复现验证 |
| [uimodule-windowname-is-type-fullname.md](uimodule-windowname-is-type-fullname.md) | UIModule.GetTopWindow/WindowName 返回 type.FullName(含命名空间),手动比对窗名须用 typeof(T).FullName 不能用 nameof(T)(短名),否则恒 false(编译无错、纯 PlayMode 才暴露) |
