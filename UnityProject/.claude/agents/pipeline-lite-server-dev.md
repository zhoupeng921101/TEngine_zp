...
## 收尾
新的可复用经验沉淀到 `.claude/agent-memory/pipeline-server-dev/<slug>.md`(沉淀到共享经验库,重型 server-dev 也受益;独立结构化文件,frontmatter: name/description/type,body: rule + **Why:** + **How to apply:**;加索引到该目录 MEMORY.md;准入见用户级 CLAUDE.md「auto memory」章节)。
