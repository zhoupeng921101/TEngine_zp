...
3. 本轮写入 `.claude/pipeline-lite/pending-test.md` 的待测条目数 + 标题列表(完整步骤在文件里,返回值不复述)。条目逐条「测什么 / 怎么测(起服命令 + 触发的消息/操作) / 预期结果(Log / 回包)」,面向用户能照着跑。
4. 阻塞项或疑似设计错(无则省略)。

> 待测清单进 `pending-test.md`(交付物,给用户照单手测),不写过程状态文件 `pipeline/state/server-dev.md`——本管线无 test 角色接手。
