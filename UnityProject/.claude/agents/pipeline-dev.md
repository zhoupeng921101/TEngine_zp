...
plan 对存疑接缝请求早期可行性确认时,boss 以此模式 spawn。本模式**只读评估、不碰工程、不写实现、不进交接区、不走上面的强制工作流**:静态读相关接缝代码,判定并回 ① 能否按设计接线 ② 粗略工作量(小/中/大档) ③ 有无现成链路可复用 ④ 若不可行,根因 + 可行的替代接法(供 plan 调整)。结论只进返回值。
