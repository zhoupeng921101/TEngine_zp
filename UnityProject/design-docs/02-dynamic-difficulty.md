...
> [!NOTE]
> **这套 DDA 是融合发牌的底层引擎(R4)**:8 算法与权重表即融合玩法的发牌底层。本篇通篇以 `Score` 为强度信号(决定 DDA 是否激活做局);融合后<mark>DDA 用哪个分量作强度信号</mark>是一个有安全默认的范围开关——默认以 `Score` 为信号(该量在融合里若恒低则 DDA 休眠在清屏窗口),可选改接经营进度量(让做局随经营推进激活)。信号源裁决与默认见 <a href="#29-gameplay-fusion::v8">29·§3.8</a> 与 <a href="#29-gameplay-fusion::decisions">29·§七#2</a>。

> [!NOTE]
> **一句话原理:** 系统维护一个隐藏变量 `dynamicWeight`(可理解为「最近有多惯着你」的累加器)。喂你简单块 → 这个值升高 → 触发「做局」档发难块;喂你难块 → 值降低 → 触发「放水」档发简单块。**一根自动收紧又松开的橡皮筋。**
...
3. **优先级覆盖层(Override)**:特定时机插队接管。当前内置两条:
...
5. **正式动态调度**:按 `dynamicWeight` + 当前分数选中一个 **tier(难度档)**,再在档内按 8 个 odds 加权抽一个算法生成方块。
6. **后处理**:分数 &lt; 10000 时套**早期屏蔽**(剔除超难大块);非清屏窗口期做**三块去重**(避免一组里出现重复图形)。

## 二、核心机制:dynamicWeight 橡皮筋 {#weight}
...
> [!NOTE]
> 采样次数 = 算法的「努力程度」。**死亡难题 320 次**采样最舍得算力,因为「逼死你」比「放水」更难找到合适组合。这些数字目前<mark class="y">硬编码</mark>在 `BlockAlgorithms.cs`,**是一个可外置到配置表的调参入口**。

## 四、配置表结构(weightcfg) {#config}
...
### 第 1 段完整 odds 表(按权重档从低到高) {#config-odds}
...
> **可优化 / 可调点:**
>
> - **EasyDiff / StraightDeathDiff / Intuition 列几乎全 0**——做了算法却没在表里启用,难度梯度其实只有「Fill ↔ Hard」两极。可考虑用上 EasyDiff 做更细腻的过渡档。
> - **翻脸太陡**:权重过 10 这条线,做局占比**从 18% 直接跳到 73%**,体感可能是「突然变态」。可在 10~30 档加一个中间梯度。
> - **采样次数硬编码**在代码里(死亡难题 320 次),建议外置到配置表,方便按机型性能/难度调。
> - 难度几乎只靠 dynamicWeight 调,**8 个分数段区分度很低**——高分玩家和中分玩家体验曲线几乎一样,可考虑让高分段整体更毒。
