...
用 demo 通关目标(完成 5 单)做一遍数走查,确认不卡死、不无脑:

| 项 | 估算 |
| --- | --- |
| 5 个常规单平均难度 | 遵订单池锯齿波,平均约 d2(折算 2 Lv1) |
...
| 订单奖励 | 8 / 单 | 主补给来源,可溢出 |
| 自然恢复 | 1 点 / 120 秒 | 现状**留口子未实装**。**补:实装时**按现实时间跨会话回复,仅回到软上限,不溢出。单局活跃节奏靠消除+订单,自然恢复只兜「久未玩重新进来有体力起步」 |
...
> - 定 8 体力 = 一单的回血,意味着「用一次锤 = 白做一单」,代价真实存在(肉疼),但不至于用不起。消除锤代价为可调数值,留待配平。
>
> 消除锤 8 体力已于 2026-06-12 拍板([§十二 #3](#11-core-loop-completion::confirm));若实测出现滥用,可上调至 12~20(单一数值,改动无连带)。

### 7.3 体力耗尽的局内行为(原稿未定,本篇补) {#energy-empty}

```text
体力不足以再落一子，且仍有手持候选块（无法落子）时：
  1. 不立即结束对局，先给玩家三条出路（按可用性排列）：
     a. 等自然恢复（若实装）—— 显示「下次回复倒计时」
     b. 花灵力祈愿补体力（§7.1，每日限次）
     c. 用本局剩余宝箱/女神奖励里若含体力，提示去领
  2. 三条路都走不通（无灵力、无恢复、无奖励）→ 弹「精力耗尽」结束面板
     （沿用现状对局结束面板的变体标题，见 09·§七）
  3. 此种结束不清空已攒图案与灵力（跨局保留）；下次进来体力已部分恢复，可续。
```
...
- 消除锤代价 = 8 体力(建议值,可调);耗尽走「三出路 → 精力耗尽结束面板」流程,跨局保留图案/灵力([§7.3](#11-core-loop-completion::energy-empty))。
...
- 完成 5 单(demo 通关目标)体力净正,不饿死([§6.5](#11-core-loop-completion::econ-balance))。
