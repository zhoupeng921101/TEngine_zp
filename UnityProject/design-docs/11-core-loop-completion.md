...
> [!NOTE]
> **降级到 demo 范围**:当前切片只有日常轨(`ActiveOrders=2`),特殊轨 = 0 槽即为现状。本节的特殊轨是**本篇新增**,落地时新增 1 个特殊槽 + 三类订单的触发器,日常轨零改动。**剧情单的「死锁防护」**见 <a href="#11-core-loop-completion::economy">§六·剧情订单</a>(终极神物必须可在合理时间内凑出)。
...
| **等级 <span class="lv">Lv1</span>–<span class="lv">Lv3</span>** | 图案的合成层级(封顶 Lv3,见 [§六](#11-core-loop-completion::economy)) | `MaxLevel=3` | 初/中/高/特/顶(原 5 级,降为 3 级) |
...
> [!NOTE]
> **「生成器」的处置**:浪漫餐厅的「生成器周期产图案」在本作不存在——本作图案产出动作就是**落子消除**(见 <a href="#09-merge-order-energy::tradeoff">09·§六</a>)。订单稿里凡「生成器/食材/仓库」一律按上表替换为「候选块/图案/收集区」,不保留 merge 概念。
...
### 7.2 消除锤 20 体力的合理性(原稿未论证,本篇补) {#hammer}
...
### 7.3 体力耗尽的局内行为(原稿未定,本篇补) {#energy-empty}

```text
体力 < PlaceCost 且仍有手持候选块（无法落子）时：
  ...
  3. 软 GameOver 不清空已攒图案与灵力（跨局保留）；下次进来体力已部分恢复，可续。
```
...
> [!NOTE]
> **与现状的关系**:R4(常规动态难度)= 工程已实现的 `dynamicWeight` 橡皮筋(见 <a href="#02-dynamic-difficulty">02 文档</a>)。R1–R3 + 优先级链是**本篇新增**的上层仲裁,套在现有发牌之外:四条特殊规则都不触发时,完全回落到现状发牌,**现状行为零改变**。这保证智能生成可作为独立增强分批落地。
...
> [!NOTE]
> **降级到 demo 范围**:女神是**本篇新增**系统。最小版 = 单档好评条(全清 +1,满 10 发一次奖),好感多级 + 赛季列后续。现状切片无女神,落地时新增计数器 + 满档弹窗(可复用 `MergeOrderWinWindow` 风格)。
...
- 连消按连续消除落子链结算,倍率 ×1.0/1.2/1.5/1.8/2.0 封顶,链断回 ×1.0;**倍率仅作用显示分,不影响 `ElementsForScore` 元素产出**(关键隔离,可单测)。
...
| 7 | **体力耗尽可花灵力祈愿补体力(每日限次)** | 引入软货币续命(去变现版续命) | 替代广告复活/付费续命的去变现方案;用玩家自攒货币([§7.1](#11-core-loop-completion::energy-soul)) | **中**。引入「灵力 → 体力」兑换通道,须防止灵力贬值/续命滥用(靠每日限次约束)。若不要此通道,体力耗尽只能等恢复或结束 |
