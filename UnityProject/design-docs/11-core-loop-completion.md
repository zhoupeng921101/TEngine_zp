...
flowchart TD
    ...
    subgraph base["底座"]
        ...
        e1["体力(节流资源)· §七<br/>落子 -1 · 消除返 · 订单 +8 · 时基恢复(含离线)"]
    end
    b1 -->|主产出链| b2
    b2 -->|主产出链| b3
    b3 -.->|资源回灌| e1
...
> [!NOTE]
> **全清还触发方块换皮(纯视觉,独立轴)**:除上表奖励外,全清事件还驱动棋盘方块皮肤切换(首次全清整盘转单色、此后每次换一张随机单色),见 [设计 50](#50-block-skin-switch)。换皮是与「换背景(背景轴)」「图案奖励(经济轴)」并列的第三条全清视觉轴,不发任何经济奖励、不参与本节结算,故不列入上表。
...
> [!TIP]
> **这是整套经济的地基,现状订单池设计已据此确定,本篇遵从并解释清楚:**
>
