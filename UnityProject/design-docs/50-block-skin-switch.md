...
| **方向约束** | 在已落地融合 / 无尽玩法之上做加法;**不引入新存储框架 / 不新增窗口**(纯棋盘渲染行为)。皮肤态续存沿用既有存档边界([设计 14](#14-save-system),当前归元层,见 §六)。 |
...
| 4 | 皮肤态跨会话续存 | 退出重进棋盘外观保持,不回退到彩色(归元层,见 [§六](#50-block-skin-switch::persist)) |
...
| 模式 | 棋盘上每个方块怎么取图 |
| --- | --- |
| **彩色** | 按方块**类型**取各自固定的 `default_skin` 纹理图——不同类型的方块显示各自那张纹理(共 8 张,见 [§五](#50-block-skin-switch::art)),与单色态同样走「贴纹理 sprite」机制(非纯色填充)。 |
| **单色** | **全盘所有方块,不分类型,统一显示「当前单色标识」指向的同一张 sprite**。这是「真正的单色棋盘」——整盘一个颜色,不是「每种类型换成另一套配色」,也不是「整套换风格」。 |

> [!NOTE]
> **两态都贴纹理,区别在「按类型分图」还是「全盘一张」**:彩色态每种方块类型贴 `default_skin` 里各自那张纹理(8 类 8 张);单色态全盘统一贴候选池里随机选中的那一张。两态渲染都经纹理 sprite,不用纯色填充——纯色块与单色纹理块并置观感不一致,故彩色态也用 `default_skin` 这 8 张专门为彩色准备的纹理。
...
| **单色** | `blocks_skin/`(目录内含 README 资产清单) | `Atlas_blocks_blocks_skin` | `blocks_skin_atlas_<编号>` | 374 张 |

**彩色态「方块类型 → 纹理图编号」对应(美术规格,8 类 1:1,看图比色已定)**:
...
- 故候选池是「这 374 个实际存在编号的集合」,**不能用** `1..477` 或 `1..374` 这类连续区间近似。
...
> [!WARNING]
> **彩色与单色不是同一套「按类型对应」的图**:彩色是 8 张「按方块类型固定配色」的图(每类型一张);单色是 374 张「全盘统一用一张」的色块库。二者不存在编号对应关系——单色态下不按类型选图,整盘只用候选池里随机选中的那一张。

## 六、续存要求 {#persist}

皮肤态需**跨会话 / 重进游戏续存**:退出重进棋盘外观保持,不回退彩色。

**存档归属层**(对齐 [设计 14 §3.1](#14-save-system::format) 进盘判据):

| 项 | 语义层级 | 进盘? |
| --- | --- | --- |
| **皮肤模式(彩色 / 单色)** | 单调只增成就标记(一旦转单色永不回退) | **是(元层)** |
| **当前单色标识** | 单色态下全盘在用那张 | **是(元层)** |

- 皮肤态归**元层**:语义是「已达成首次全清」的单调只增标记(规则 4 单色不退彩色),与最高分 / 解锁集等元层只增字段同类,不同于每局重建的棋盘 / 手牌。
  > 续存归属随玩法模型演进:无尽模型([设计 49](#49-infinite-no-rounds))「局内态整盘续存」通道落地后,皮肤态可随整盘续存并入局内段(字段语义不变、可平移);在该通道落地前,元层是唯一跨会话持久通道,且单调标记语义本就贴合元层。
...
| 功能点 | 档 | 说明 |
| --- | --- | --- |
| 彩色态(按类型贴 default_skin 纹理)= 初始 | <span class="pill-core">核心</span> | 初始皮肤态,8 类各贴各自纹理图(§五 映射)。 |
...
| # | 验收点 | 完成定义(人眼 / 截图核对) |
| --- | --- | --- |
| B1 | 全新进游戏彩色态贴纹理 | 全新存档进游戏,棋盘方块按类型显示各自 `default_skin` 纹理图(不同类型显示不同那张,8 类对应 §五 映射),非纯色填充。 |
...
| **续存归错通道**(不进盘 / 归错段) | [§六](#50-block-skin-switch::persist) 明确归元层(单调只增标记),对齐 [设计 14](#14-save-system) 元层段;验收 A5 续存往返保真覆盖。 |
