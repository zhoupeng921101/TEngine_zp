...
| O5 | 头像 EVENT 解锁通路 | **不在本子单** <span class="pill-cut">Tier 4 第 2 子单</span>:`AvatarUnlockService.GrantUnlock` 是客户端进程内 API,需扩 16 道具系统 `UseEffect` 加「解锁头像 N」效果使礼包→头像解锁通路打通 | Tier 4 第 2 子单接;本子单 `activity.reward` 仍可挂礼包 id(`giftrandom` 已有的库),待 16 扩 `UseEffect` 后增加「解锁头像」类库 |
