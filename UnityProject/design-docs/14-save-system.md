...
| **方块皮肤态(彩色 / 单色 + 当前单色标识)** | 单调只增成就标记(一旦首次全清转单色不回退,见 [设计 50](#50-block-skin-switch)) | **是(元层)** |
| —— 以下不进盘(纯内存瞬态)—— |  |  |
...
| 元层 | 方块皮肤态(皮肤模式 + 当前单色标识) | int/enum + string/int | 棋盘外观,随全清演进([设计 50](#50-block-skin-switch));缺字段保底为彩色态 |
