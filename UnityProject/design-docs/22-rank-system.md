...
>
> - **「服务器接缝」= 可注入排名数据源接口,不是真拉全服排名。**本作**没有网络层**,方向**离线还原 · 去变现**。「全服排名 / 攀比」本应由服务器汇总各玩家数据后下发,本设计抽象成一道排名数据源接口:离线默认本地源——**用本机自己的一条成绩 + 配置陪榜垫底,本地排序产出一份可跑可测的榜单**(玩家能看到自己名次、能结算发奖);远程源仅留**占位实现**(不连网、返空 / 无副作用)。这道接缝指的是**排名数据从哪来**这个边界本身,而非本设计真去连服务器(见 [§3.6](#22-rank-system::source) / [§六 O1](#22-rank-system::open))。
...
> | **方向约束** | 离线还原 · **去变现**:排行榜用于**进度显示 / 攀比 / 名次奖励**(设计目的三条),**不**含充值榜 / 付费冲榜 / 买名次;真实全服排名延后(本作无网络层),离线版远程源为占位,本地源用本机成绩 + 配置陪榜产出可玩可测的本地榜。本地存档用非阻塞写入(不触「禁阻塞 IO」红线)。加法式扩展,不破坏既有核心循环 + 已建系统(尤其邮件系统 21 零改动,只调用)。 |
...
| 13 | 全服真实排名(服务器侧) | 排名数据源接缝:离线本地源(本机成绩 + 配置陪榜,可跑可测)/ 远程**占位源**([§3.6](#22-rank-system::source) / [§六 O1](#22-rank-system::open)) | <span class="pill-no">远程占位</span> |
...
**不做(本设计明确排除):**<span class="pill-no">真实全服排名 / 服务器拉榜</span>(无网络层,O1);<span class="pill-no">真实他人玩家数据</span>(离线无,陪榜由配置生成,O2);<span class="pill-no">所有 UI 窗口</span>(界面 / 列表 / 点赞按钮 / 头像 — 需美术,O8);<span class="pill-no">多语言名称 / 文案真实查表</span>(文本 id 占位,同 num/item/reward/mail 现状,O6);<span class="pill-no">道具 / 跑马灯</span>(设计明写无,O7);<span class="pill-no">充值榜 / 付费冲榜 / 买名次</span>(去变现方向,不做)。

## 二、数据模型与分层 {#model}

排行榜底层分三层,职责清晰互不越界:**配置层**(榜定义 + 奖励档,只读)、**数据源层**(榜上有谁、各多少分 — 接缝,离线本地 / 远程占位)、**服务层**(查榜 / 排序并列 / 结算编排 / 领取 / 红点 — 纯逻辑)。结算时服务层向**邮件系统 21** 借发奖入口发奖。结构图:
...
### 3.6 排名数据源接缝(本地可测 / 远程占位) {#source}

这是标题「服务器接缝」的实义。「全服排名」本应由服务器汇总下发,本作无网络层。抽象成一道**排名数据源接缝**:给一个榜 id,返回「榜上有哪些参赛记录」(原始未排序),由服务层排序。接缝产出的每条参赛记录含玩家展示名、成绩、达到时间、是否本机玩家。两个实现:

- **离线本地源**(可跑可测):本机最佳成绩(来自持久化 / 注入)+ 配置陪榜成绩 → 一组参赛记录,本机那条标记为本人。「陪榜」= 配置 / 注入的基准成绩(NPC 名 + 分数文本 id 占位),使单机也有一份可排序的榜。
- **远程占位源**:无服务器,返空、不连网、无副作用。未来上后端时实现一次(拉全服榜 → 转成参赛记录),服务层与排序逻辑零改动。

**为什么离线游戏还要这道接缝?** 把「排名从哪来」一次划清:离线注本地源(本机 + 陪榜,玩家有得玩、有名次、能结算);未来上后端只换远程源实现(拉全服真实榜),<mark>查榜 / 排序 / 结算 / 领取逻辑零改动</mark>。陪榜成绩本设计由**配置 / 注入**提供(基准分,NPC 名占位文本 id);不引入随机生成 NPC(那是表现层 / 运营内容,O2)。这与去变现方向不冲突——榜只比成绩,不卖名次。
...
| 接缝 SK | SK1 | 远程占位源取记录返空集合且**不抛、不连网**;本地源取记录返「本机一条(本人)+ 注入陪榜」可被服务排序;全系统无真实网络调用 |
...
| 回归 / 编译 R | R2 | Code Review 5 红线:异步优先 / 模块访问规范 / 资源释放 / 热更边界 / 事件解耦(本层无资源加载、无事件;重点核「无真实网络调用」「本地存档非阻塞不触同步 IO」「结算发奖复用邮件系统 21 不另造发奖、不碰玩法状态」「持久化复用既有本地存档不另造存储栈」「邮件系统 21 零改动」) |

> [!WARNING]
> **不在本设计验收**
>
> 真实全服排名(无网络层)、真实他人玩家数据(离线无,陪榜配置生成)、排行榜界面 + 名次列表 + 我的名次条 + 点赞按钮 + 奖励预览 + 头像框 UI 视觉、排行榜 icon 红点显示、主界面入口接线、结算的自动触发时机(登录检查 / 后台轮询由表现层 / 流程层接) → <mark>表现层延后轮 + 远程实现未来轮</mark>。依赖美术(UI)与后端(服务器),数据层不返工。
...
| # | 开关 | 本设计默认 | 备选 / 触发改动 |
| --- | --- | --- | --- |
| O1 | 真实全服排名 / 服务器拉榜 | **远程占位源**(返空、不连网);离线用本地源(本机 + 陪榜,可跑可测) | 未来上后端实现远程源(拉全服榜 → 参赛记录),排序/结算/领取零改动 |
...
| 风险 | 应对 |
| --- | --- |
| **误把「服务器接缝」做成真拉全服排名**:引入真实网络请求或真连后端 → 离线游戏跑不通、且违方向 | 读前必看第 1 条 + §3.6 已明确:接缝 = 可注入数据源接口,离线注本地源(本机 + 陪榜),远程占位源仅返空。Code Review R2 + 验收 SK1 核「无真实网络调用」 |
