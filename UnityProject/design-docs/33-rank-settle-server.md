...
> - **本刀不动客户端、不动设计 22 正文。**server 段先行,客户端的本地结算编排([设计 22 §3.5](#22-rank-system::settle))在客户端段退役前仍真、仍可跑;本篇 [§六](#33-rank-settle-server::seam) 前瞻记录客户端段「本地结算退役」适配契约,真正触发设计 22 现状改写的是后续客户端段增量(同 [31 对 22](#31-rank-server::seam) / [32 对 21](#32-mail-server::seam) 的处置)。
...
## 六、与设计 22 的关系(结算上移 + 客户端段适配) {#seam}

[设计 22 §3.5](#22-rank-system::settle) 的结算编排是**客户端进程内纯方法**(登录 / 主循环触发,算本机名次,经客户端邮件入口发奖,本地存上次结算标记)。本篇把结算上移服务端。**本增量 server 段先行,只建服务端结算 + 幂等,未改客户端现状**,故设计 22 本次**无需改写**(其客户端本地结算在客户端段退役前仍真、仍可跑);下表为**客户端段(后续增量)**的适配前瞻,客户端段据此开工:

| 设计 22 现状 | 本篇后(客户端段适配) | 处置 |
| --- | --- | --- |
| 结算编排在**客户端**(算本机名次 → 客户端邮件入口发奖 → 本地存已结标记,§3.5) | 结算上移服务端;客户端**不再本地算名次发结算奖** | <mark>客户端段关键改写点</mark>:客户端本地结算编排退役(同 [设计 32](#32-mail-server::seam) 领奖上移、[31](#31-rank-server::seam) 远程源兑现);设计 22 的「结算纯方法 + 本地已结标记」退出权威路径 |
| 结算奖经**客户端**邮件入口(设计 21 对外收件接口)发本机一封 | 结算奖经**服务端**发奖入口([设计 32 §3.5](#32-mail-server::source-api))给上榜账号发;客户端只在邮箱**领**(走设计 32 客户端段领奖) | 客户端「发结算奖」退役,「领结算奖」走设计 32 领奖链;两个邮件入口(客户端进程内 vs 服务端进程内)区别同 [设计 32 §六](#32-mail-server::seam) |
| 查榜 / 排序 / 我的名次 / 提交成绩 / 每日点赞 / 红点(设计 22 §3.3/3.4/3.7) | **本刀不动**;查榜 / 我的名次走 [设计 31](#31-rank-server) 远程源(已由设计 31 兑现) | 结算外的服务层不受本刀影响;每日 / 点赞奖上移留后续(O1) |
| 本机元层进度持久化:上次结算时间 / 已结标记(设计 22 §3.8) | 结算上移后,**已结标记权威在服务端**;客户端本地不再存「全服榜已结标记」 | 客户端本地存档退化(不再承载结算幂等);本机最佳分仍可本地缓存(查榜降级用,设计 31 本地源) |

> [!NOTE]
> **为什么本增量不改写设计 22?(与 31/32 同理)**
>
> 本增量是 server 段先行刀,只建「服务端结算 + 幂等」,**未改客户端现状**——设计 22 的客户端本地结算在客户端段落地前**仍真、仍可跑**(离线 / 断服时客户端仍可按本地源结算本机名次发本机邮件,作降级)。真正触发设计 22「客户端结算 → 服务端结算」现状改写的是**后续客户端段增量**(同 [设计 31](#31-rank-server::seam) / [32](#32-mail-server::seam) 对旧稿的处置:server 段先行不急着改旧稿,先问「这一刀改的是哪一端的现状」)。本节为前瞻记录客户端段适配契约,供其开工;本增量不改设计 22 正文。

> [!NOTE]
> **排行榜全栈三刀的收口:31 数据源 + 33 结算 + 客户端段**
>
> 排行榜上后端按「数据源 → 结算 → 客户端切换」三刀走:[设计 31](#31-rank-server) 建全服分数存储 + 排序 + 名次(上报 / 查榜协议);**本篇(33)** 建服务端结算(到点 → 算名次 → 档奖 → 发结算邮件 + 幂等),复用 31 的分数 + 32 的发奖入口;客户端段(后续)把设计 22 客户端的远程源占位切真实 RPC(对应 [设计 31 §8.2](#31-rank-server::accept-client) CV)+ 本地结算退役(对应本篇 §六)。三刀都 server 段先行、客户端段后随,设计 22 / 21 客户端正文在各自客户端段落地时统一改写。本刀是服务端侧的最后一块(数据源 + 结算齐了),客户端段是把客户端从离线占位切到这套服务端权威上。
...
| **误改设计 22 客户端正文**:server 段先行却急着改设计 22 | [§六](#33-rank-settle-server::seam):本刀只建服务端、未改客户端现状,设计 22 客户端结算在客户端段退役前仍真;本刀不改设计 22 正文,§六前瞻记录客户端段适配(同 31/32 处置) |
