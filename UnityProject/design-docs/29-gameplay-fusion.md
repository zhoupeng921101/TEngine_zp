...
> | **已锁定决策（不再讨论）** | ① 融合产物 = 一份统一设计文档；② 经典**完全吸收**，移除「纯无尽」独立入口；③ 单局**保留体力预算**作为约束。见 [§七](#29-gameplay-fusion::decisions) |
...
| **合成订单** | 落子消除 → 产图案 → 自动合成 → 交订单换灵力 / 虔诚币，带体力预算、盲盒、女神、神庙；完成 5 单通关或体力耗尽软 GameOver |
...
| 合成订单（11） | 三条：完成目标 5 单 → 通关；体力付不起落子且无单可交付 → 软 GameOver「精力耗尽」；手牌无处可放 → 硬 GameOver |
...
| 合成订单（11） | 局内不刷得分（落子 / 消除均不累加可见分）；用消除基础分作**元素产出的内部驱动量**，不计入玩家可见分；交付累计「交付分」作通关 / 结算摘要。结束时把交付累计一次性镜像给可见分供结算窗显示 |
| <span class="verdict">融合后取定</span> | **三个量各司其职、口径写明，不强行合并**：<br>① **消除基础分** = 驱动量，喂元素产出与连消倍率，**不是玩家可见的「真分数」**；<br>② **交付累计分** = 经营成绩，通关 / 结算摘要；<br>③ **最高分** = 经典遗产，跨局存储的长期挑战指标。<br><mark class="y">融合后须明确「玩家可见的主分数」口径</mark>，并理清局内可见分在融合玩法的角色（详见 <a href="#29-gameplay-fusion::dev">§6.3</a> 隐患 A） |
...
| 合成订单（11） | 元层跨会话存档（设计 14）：灵力 / 虔诚币 / 经验·守护者 / 神庙 / 盲盒 / 女神 / 完成单数 / 今日祈愿等 13 项。局内瞬态（棋盘 / 手牌 / 订单 / 合成区 / 悔棋栈）**不进盘**，每局重建 |
| <span class="verdict">融合后取定</span> | **合并为一套存档视图**，但**保持「元层进盘 / 局内瞬态不进盘」的分层不变**（设计 14 已确立的判据：跨局累积进盘、局内瞬态每局重开）。融合后单局态 = 棋盘 / 手牌 / 分数 / 体力 / 合成区 / 订单；元层 = 灵力 / 虔诚币 / 女神 / 神庙 / 最高分等。经典的最高分并入元层语义（跨会话长期指标），与元层进度**同一套加载 / 落盘时机** |

> 核心是把最高分的跨会话存储与元层归一时机，并明确单窗口下「局内续玩」用哪套（设计 14 的元字段导入只覆盖元层、不触局内瞬态，融合沿用）。详见 [§6.4](#29-gameplay-fusion::dev)。
...
- **本篇目标**：融合为**一套窗口**承载完整循环（落子 + 消除 + 体力 + 合成 + 订单 + 叠加奖励层 + 结束判定）。实现路径（dev 取舍）：**以合成订单窗为主体**（它已是完整循环的超集），把经典独有的视觉（如大居中分数，若融合保留可见分）按需并入；或反向。<mark class="y">注意经典窗已承载塔罗木质换皮（设计 27）</mark>——合一时美术换皮归属须一并理清（哪套视觉是融合窗口的最终皮）。
...
>
> **现状（不是当前的活 bug）**：连击与局内可见分是窗口间共享的状态。两窗进入时都把它们清零，故跨窗口切换**未表现出残留 bug**——下一个窗口的进入重置掩盖了上一个窗口的退出未清。退出合成订单时本身**不**清连击 / 可见分。
>
> **本篇目标**：融合为单窗口后，「进入即重置」仍须是**显式不变量**（融合窗口的进入路径必须清零局内瞬态分量）。同时确定三个分量的最终角色：消除基础分（驱动量，不可见）/ 交付累计分（经营成绩）/ 最高分（跨会话）；连击降为视觉镜像。**连带项**：早期屏蔽阈值（<a href="#29-gameplay-fusion::v6">§3.6</a>）依赖的「可见分口径」随此一并定。
...
- **现状**：两套存档并存——经典局内存档（棋盘 / 手牌 / 分数 / 最高分）、动态难度权重单独存一份、以及元层存档（设计 14，灵力 / 虔诚币 / 女神 / 神庙 / 盲盒 / 完成单数等）。
- **本篇目标**：合并为**一套连贯的存档视图**，**保持「元层进盘 / 局内瞬态不进盘」分层**（设计 14 判据）。核心动作：① 把经典最高分的跨会话存储并入元层时机（与元层存档同一加载 / 落盘节点）；② 明确融合单窗口的「局内续玩」用哪套（元字段导入只覆盖元层、不触局内瞬态与悔棋栈，融合沿用此分层）；③ 动态难度权重的存储随隐患 B 的归一策略定（若每局重置则不必跨会话存）。
...
- 明确单入口（[§3.1](#29-gameplay-fusion::v1)）、体力优先的三出口结束模型（[§3.2](#29-gameplay-fusion::v2)）、连消钩子点亮 = 显示分倍率链（[§3.5](#29-gameplay-fusion::v5)）、发牌统一为底层 DDA（[§3.4](#29-gameplay-fusion::v4)）。
...
- **三隐患按选定策略落地**：进入融合窗口清零局内瞬态分量（隐患 A）；动态难度权重归一策略已显式实现并写明（隐患 B）；DDA 强度信号口径已定（隐患 C）。
- **存档合并**：元层 / 局内瞬态分层不变；最高分跨会话存储与元层同时机；融合后存读档往返不丢元层进度（沿用设计 14 单测口径）。
...
| 3 | **动态难度权重跨局归一策略**（[§6.3 隐患 B](#29-gameplay-fusion::dev)） | **默认每局重置**：融合开局做完整重置，每局公平起步（消除现状「跨局 / 跨模式 / 跨重启累积」的隐式行为，最可预测） | 刻意跨局累积：保留橡皮筋长期记忆玩家水平（需说明跨会话存储语义） | 小。单处开局行为；不改 8 算法 / 权重表 |
...
> [!NOTE]
> 四项默认**组合自洽**：以合成订单为主体（#4 默认）、DDA 维持现状信号在融合里休眠（#2 默认）、动态难度权重每局重置消除残留（#3 默认）、R1–R3 暂不接（#1 默认）——构成「最小变更、零回归、行为可预测」的安全融合基线。任一项改备选都是**局部增量**，不返工已落地系统。
...
| 风险 | 应对 |
| --- | --- |
| 两窗合一改坏在跑的玩法（核心玩法窗每局全程在跑，改一处坐标 / 逻辑即整局回归） | §5.2 立零回归红线：棋盘坐标常量绝不动、拖拽 / ghost / 落子判定保留；以合成订单窗（完整循环超集）为主体减少新写 |
| 三隐患若按「直觉重置」一刀切，可能误删刻意的跨局累积（如动态难度权重长期记忆） | 三隐患均**显式列为「须选定并写明」**而非默认改重置；§七 #3 给默认 + 备选，由 boss 拍 |
