...
一套**收件箱数据逻辑层**:邮件数据模型(发件人 / 时间 / 标题 / 内容 / 奖励附件 / 已读 / 已领取)+ 收件箱服务(收件 / 列表排序 / 标记已读 / 领取奖励单封+一键 / 删除已读 / 自动清理超量+过期 / 红点)。关键在两道接缝:① **对外收件接口**——供排行榜结算 / 活动回收 / 系统奖励调用(即 spec 的「为其他功能留邮件调用接口」,客户端进程内,离线可用);② **服务器/运营接缝**(运营推送来源)<mark>已接 [服务端](#32-mail-server)</mark>(运营邮件权威存服务端,客户端拉应收列表 + 领奖走服务端校验,断服降级空载)。奖励附件复用 [道具系统 16](#16-item-system) 的礼包随机库 + 发奖落点(服务端抽奖、客户端本地落地),领取后用 [设计 17](#17-reward-display) 的统一奖励展示。这是 xlsx 系统底层批次第七刀。<mark>邮件界面 / 详情 / 红点显示 / icon(表现层)需美术,延后轮,本设计只留服务 + 接缝 + 红点状态查询。</mark>
...
>
> - **「服务器/运营接缝」= 可注入的邮件来源接口(占位 / 远程可换)。**方向**离线还原 · 去变现**。「后台全服发邮件 / 定时邮件 / 区服多选」由运营后台 + 服务器推送——[设计32邮件上后端](#32-mail-server)已将邮件来源接真实 RPC(服务端运营推送 + 领奖权威),客户端拉收件箱 / 领取走服务端;**断服回退占位**(返空、不阻断)。**区服离线视单一本地区服**(无多区概念)。这道接缝指的是**邮件从哪来**(占位 / 远程)这个边界本身(见 [§3.7](#21-mail-system::source) / [§七 O1](#21-mail-system::open))。
...
| 13 | 运营/服务器:后台发删定时邮件(标待定/后面做) | 邮件来源接缝已接 [服务端](#32-mail-server)(运营推送来源 + 领奖权威),客户端拉应收列表 + 领奖走服务端校验,断服降级空载;区服离线视单一本地区服([§3.7](#21-mail-system::source) / [§七 O1](#21-mail-system::open)) | <span class="pill-cur">已接服务端</span> |
...
> [!NOTE]
> **领奖裁决权威已上移服务端。** 运营邮件的领奖(同邮件同账号一次的防重、附件奖励抽奖、过期判定)已由 [领奖上后端](#32-mail-server) 移到服务端裁决:客户端发领取请求 → 服务端原子防重 + 按附件库 id 抽奖 + 服务端时钟判过期 → 仅「成功」时客户端按服务端裁定的奖励本地落地(复用 16 落点)。本节描述的本地抽奖 / 本地防重不再是领奖权威路径(同 [设计 30](#30-redeem-code-server::supersede) 兑换码本地校验退出权威);领奖断服**不本地放行**(可重复领 = 超发,[设计 32 §四](#32-mail-server::degrade))。下文的结果码 / 文案 / 一键逐封编排在客户端段保留,服务端化新增「服务不可用」分支。

领取结果 = 抽奖励库 → 落点 → 标已领 → 标已读(权威已上移服务端,见上注)。复用 16 礼包库+落点,产出一组奖励结算项供 UI 转交 17 的统一奖励展示。

**领取结果**包含三部分:结果码、结果文案占位标识(§3.8)、成功时的奖励结算项列表。结果码分五种:成功 / 邮件不存在 / 无奖励 / 已领过 / 已过期(服务端化新增「服务不可用」第六类,[设计 32 §3.4](#32-mail-server::claim-resp))。
...
这是标题里「服务器/运营接缝」的实义。spec 的「后台全服发邮件 / 定时邮件 / 区服多选」由运营后台 + 服务器推送。本设计把它抽象成一个**邮件来源接口**(拉取该账号应收的邮件),来源接口背后已接 [运营推送来源服务端化](#32-mail-server)(运营邮件权威存服务端,客户端经联网会话拉应收且未过期的邮件列表)。

- **邮件来源接口**:对外暴露一个「拉取该账号应收邮件」的能力。运营在服务端配置的全服广播 / 系统定向邮件经此进收件箱(身份从联网会话取,客户端不自报账号,详见 [设计 32 §3.1](#32-mail-server::pull))。
- **断服降级**:拉列表不可达(发不出 / 超时 / 服务不可用)时空载(空列表 / 提示稍后重试),不阻断玩法、无本地运营邮件可伪造(运营来源唯一在服务端,[设计 32 §四](#32-mail-server::degrade))。区服离线视单一本地区服(无多区概念)。

**这道接缝与对外收件接口的区别。** 对外收件接口是<mark>游戏内系统</mark>发奖入口(排行榜客户端结算 / 活动 / 补偿,客户端进程内,离线可用,本设计真做);邮件来源接口是<mark>外部运营 / 服务器</mark>推送入口(运营邮件来源,[设计 32](#32-mail-server) 服务端权威)。两道接缝一进一出,各自独立:对外收件接口仍是客户端进程内能力;邮件来源接口已上后端。这与去变现方向不冲突——服务端只下发运营配置的奖励,不发可购买物。
...
| 接缝 SK | SK1 | 邮件来源**占位 InertMailSource** 拉取返空集合且**不抛、不连网**(占位回退路径;真实 RPC 往返在 RemoteMailSource,见[设计32](#32-mail-server));数据层 EditMode 单测以桩响应 / 占位覆盖、不连真网 |
...
| # | 开关 | 本设计默认 | 备选 / 触发改动 |
| --- | --- | --- | --- |
| O1 | 真实服务器后台发删/定时邮件 | **已接服务端**(邮件来源接口背后接 [设计 32](#32-mail-server) 运营推送来源 + 领奖权威;断服降级空载) | 已实现来源接口(拉服务端应收邮件 → 收件箱)+ 领奖走服务端校验;收件箱表现(排序/已读/删除/红点/清理)仍客户端 |
...
| 风险 | 应对 |
| --- | --- |
| **远程接真实 RPC 后漏断服降级 / 领奖本地放行**:断网时邮件不可用或卡;领奖断服本地放行 → 可重复领超发 | [设计32](#32-mail-server):列表断服空载降级、领奖断服**不本地放行**(返服务不可用、进度不变,同兑换码);占位 InertMailSource 仍返空(断服回退路径)。对外收件接口 = 游戏内发奖入口(本地,本设计真做)与邮件来源(外部推送,设计32 接真实)区分。Code Review R2 + 验收 SK1 核「数据层不连真网 / 占位不连网 / 领奖不本地放行」 |
