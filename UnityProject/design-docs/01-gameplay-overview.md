...
经典 <mark>8×8 棋盘 + 三块拖放消除</mark>(同类:Block Blast / 1010!)是整套玩法的**底层引擎**:落子 → 行列消除 → 补牌,这条裸循环全程在跑。融合后它**不是整个游戏的唯一循环**——其上叠了体力、图案产出、订单交付。无尽模型([设计 49](#49-infinite-no-rounds))下**无任何结束出口**:无通关、无 GameOver,卡死与体力归零由两条兜底脱困。下图分两层:内层蓝框是经典裸循环(底层引擎),外层是融合上层与两条兜底。

```mermaid
flowchart TD
    subgraph eng["经典底层引擎(裸循环 · 全程在跑)"]
        h1["手牌 3 块<br/>调度发牌 · §四"]
    ...
    stam["体力(节流资源)· 融合上层<br/>落子 -1 · 消除返 · 订单回灌<br/>+ 时基恢复(含离线)· 设计 49"]
    pat["产图案 / 合成 / 交订单(无限)<br/>消除驱动 → 换灵力 · 见 11"]
...
> [!NOTE]
> **无尽模型取代经典「无处可落即唯一结束」**:融合后运行在无局 · 无尽模型下([设计 49](#49-infinite-no-rounds))——**无通关、无软/硬 GameOver**。经典的「剩余手牌无合法落点 → GameOver」被<mark>覆盖移除</mark>:卡死不结束游戏,由消除道具清块脱困([49·§3.1](#49-infinite-no-rounds::cleartool));体力归零不结束游戏,由时基恢复(含离线)迟早回升([49·§3.2](#49-infinite-no-rounds::regen))。脱困闭环见 <a href="#49-infinite-no-rounds::deadlock-proof">49·§3.3</a>,融合裁决见 <a href="#29-gameplay-fusion::v2">29·§3.2</a>。

窗口流转:主菜单(单入口「开始游戏」)→ 游戏窗(无尽循环,无结算 / 通关窗——游戏不主动结束,[设计 49](#49-infinite-no-rounds))。两窗合一与单入口化属融合落地项,见 [29·§五](#29-gameplay-fusion::dev)。
...
- **方块外观分两套皮肤**(形状之上的渲染层):初始彩色(每种类型固定配色)→ 全清后转单色,见 [设计 50](#50-block-skin-switch)。皮肤只影响外观,不影响形状库 / 发牌 / 玩法。
...
- **存档**:经典存档(棋盘 + 手牌 + 分数 + 最高分)与动态权重自存是底层持久化。无尽模型下并入**整盘续存**的统一存档视图([设计 14](#14-save-system)):元层 + 局内态(棋盘 / 手牌 / 合成区 / 订单 / 体力)都进盘,退出重进从原状态继续,最高分并入元层语义(跨会话长期指标)。合并裁决见 [29·§3.7](#29-gameplay-fusion::v7)。
