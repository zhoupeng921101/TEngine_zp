...
> - **本子单不做的事(显式排除,留下一刀 / 后续 Tier):**ledger 历史**写入侧**已交付,见 [设计 44](#44-player-attr-ledger)(`player_attr_ledger` 集合 + source 枚举 + (account, timestamp) 索引,在 37 「FindOneAndUpdate 成功」分支后旁路追加);查询 API + 客户端「我的流水」UI 留 Tier 2+ 客户端段后续刀。商店购买路径接通(商店发起消费由商店系统某刀接 PropertyChangeRequest 服务端进程内 API,本刀不接)、活动 / 任务 / 排行榜结算的奖励发放路径接通(各业务系统的发奖刀按其各自范围接 PropertyChangeRequest 进程内 API,本刀仅声明该入口形态)、退款 / 回滚(Tier 2+,需 ledger 才能做)、客户端 PlayerPrefs 迁移(客户端段下一刀)、Player 模块改持有方式(客户端段下一刀)、其它属性(经验 / 等级 / 体力上限 / 体力恢复速率等非本三属性)。
...
| ledger 历史流水查询(写入) | **服务端 · 44 已交付** | 见 [设计 44](#44-player-attr-ledger):`player_attr_ledger` 集合 + source 枚举 + (account, timestamp) 索引,挂 37 「FindOneAndUpdate 成功」分支后旁路追加 |
| ledger 历史流水查询(客户端拉 + 我的流水 UI)| **不做** <span class="pill-cut">Tier 2+</span> | 留 Tier 2+ 客户端段后续刀,与 44 server 段独立演进 |
...
| 末次变更时间 | 日期时间 | 该 UUID 最后一次属性变更的时刻(insert 时同首登时间;后续每次变更 update);**44 已交付 ledger 后**,完整流水维度由 `player_attr_ledger` 承载,本字段仍保留作 `players` 单文档最简单标记(高频读不放大,与 ledger 集合各司其职) |
...
**为什么发奖入口与玩家声明入口共用一套?**两个理由:① 一套校验 / 一套写库 / 一套推送,代码重复最小、行为一致最高;② 反作弊红线统一——无论变更来自客户端声明还是服务端业务系统,都过同一套余额下界 / 上界 / 原子写,业务系统无法因「内部调用」绕过校验(例如:排行榜结算服务端的 bug 试图发奖 +999999999 钻石,会因类型上界校验失败而拒,客户端不会拿到非法值)。**[44 ledger 旁路写](#44-player-attr-ledger)亦挂在此通路成功裁决后分支**,业务系统调用方无感、ledger 自动覆盖所有发奖 / 扣减路径(只需 reason 字符串约定登记 source 枚举,见 44 §3.3)。
...
- **ledger 历史 / 流水查询**:本子单仅持单字段「末次变更时间」(高频读),完整流水**写入侧已由 [44](#44-player-attr-ledger) 交付**(`player_attr_ledger` 独立集合);玩家「我上次扣了什么 / 何时」类查询的 RPC + UI 留 Tier 2+ 客户端段后续刀
- **退款 / 回滚**:本子单的任何变更**写入即定**,不支持「3 秒内撤销」;Tier 2+ 退款刀借 44 ledger 找原笔 + 同笔反向 + source=Refund 标退款关联
...
| ledger 历史流水查询 | **44 已交付**写入侧:`player_attr_ledger` 集合(MongoDB ObjectId 主键,字段 = 应用端 timestamp + account + kind + balanceBefore + balanceAfter + delta + source 枚举 + reasonRaw + schemaVersion;索引 (account, timestamp DESC) 复合 + (timestamp DESC) 单字段;source 枚举覆盖现有所有发奖 / 扣减路径 + Unknown 兜底)。查询 API + 客户端「我的流水」UI 留 Tier 2+ 客户端段后续刀 |
...
- **ledger 历史(写入)** → **[44 已交付](#44-player-attr-ledger)** server 段;查询 RPC + 客户端 UI / 退款 / 回滚 → Tier 2+ 后续刀
