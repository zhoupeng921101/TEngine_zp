...
| 客户端业务接入(`RemoteAttrLedgerService`)+ 我的流水 UI | **客户端 · 46 已交付** | `RemoteAttrLedgerService`(沿 32 `RemoteMailService` 编排范式)+ 独立窗 `PlayerAttrLedgerWindow`(沿 28 排行榜窗范式)+ source 整数 → 中文文案 switch + 入口按钮挂 PlayerInfoWindow 改名面板下方;详见 [设计 46](#46-player-attr-ledger-client) |
...
- **客户端可查 ledger**:协议契约 + 服务端 handler + 客户端协议生成物 = 45 已交付;客户端业务接入(`RemoteAttrLedgerService`)+ 我的流水 UI 投放 + source 整数 → 中文文案 = **46 客户端段已交付**(详见 [设计 46](#46-player-attr-ledger-client))
...
| --- | --- |
| 客户端「我的流水」UI + 拉流水 RPC | **服务端段 + 客户端协议生成物 = 45 已交付**(协议 `C2G_QueryAttrLedger(kind?, sinceTs?, limit)` + `G2C_QueryAttrLedgerResponse(resultCode + entries[] + hasMore)`,handler 读 `(account, timestamp DESC)` 主索引取前 limit 条 + kind 过滤 + sinceTs 滑动窗口 + limit 上限钳制 + 字段裁剪 7 字段白名单);**客户端段 = 46 已交付**(`RemoteAttrLedgerService` 沿 32 `RemoteMailService` 编排范式 + 独立窗 `PlayerAttrLedgerWindow` 沿 28 排行榜窗范式 + source 整数 → 中文文案 switch + 入口按钮挂 PlayerInfoWindow 改名面板下方;首屏 50 条最新,翻旧页留 45 O3 Tier 2+ 协议加 `untilTs`),详见 [设计 46](#46-player-attr-ledger-client) |
...
- **客户端「我的流水」UI / 拉流水 RPC** → 协议契约 + 服务端 handler + 客户端协议生成物 = 45 已交付;客户端业务接入(`RemoteAttrLedgerService`)+ UI 投放(我的流水窗口)+ source 整数 → 中文文案 = **46 客户端段已交付**(详见 [设计 46](#46-player-attr-ledger-client))
