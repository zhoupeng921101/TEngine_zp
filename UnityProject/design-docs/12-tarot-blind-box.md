...
盲盒持有计数纳入**单局悔棋快照**,与灵魂/女神好评等单局进度同体例——落子触发的计数变化可随悔棋一并回滚,单局内回滚正确。

主玩法状态(灵魂、女神好评、盲盒计数等)整体目前**不做跨会话磁盘持久化**——存盘只覆盖棋盘、待选块、分数,主玩法状态每局重建。盲盒计数跟随此现状:只入悔棋快照,不单独建磁盘序列化层。若要主玩法状态全量跨会话存盘,那是独立的大改(全字段序列化),不在本设计、见 §七待拍板。
...
| O3 | **主玩法状态是否做全量跨会话磁盘持久化。**现状只快照、不存盘。 | 本设计不做——跟随现状只入快照。全量存盘是独立大改,单列任务。 |
