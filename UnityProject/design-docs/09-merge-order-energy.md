...
> [!TIP]
> **被清元素的去向**：被清元素路由进合成区，成为合成与订单经济的输入。「消除→统计被清元素」逻辑是 `BlockGameState.HarvestClearedElements`，输出本次被清元素列表供合成区摄入。
...
- **交付（手动）**：当合成区满足某单要求（库存有 ≥数量 的对应 `(类型,等级)`），该单「交付」按钮点亮；点击 → 扣除对应合成物 → 发奖 → 该订单槽刷新下一单。手动交付保留玩家「先交哪单」的取舍，且交付时机清晰可测。
...
> [!NOTE]
> **demo 简化实现**：用一张**手编循环订单池**预先编排好锯齿波节奏（如 易→易→中→易→难→易…），`NextOrder()` 顺序取下一项。完整的程序化锯齿波（依玩家进度动态算 `d` 并约束相邻单一高一低）规则写在本节，dev 先实现循环池版本即可满足闭环验收。
...
> [!NOTE]
> **回归红线**：本切片全部新逻辑由 `MergeOrderMode` 门控。off 时 `BlockGameState` 的 Classic 落子/消除/补块/存档与现状<mark class="y">逐字节一致</mark>。Classic 回归测试必须通过。
