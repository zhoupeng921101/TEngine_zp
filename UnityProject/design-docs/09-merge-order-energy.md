...
| 运行模型 | 无局 · 无尽循环（[设计 49](#49-infinite-no-rounds)）：订单无限、无通关、无 GameOver；卡死与体力归零由两条兜底脱困（消除道具 + 时基恢复） |
...
- **无失败条件、无终点**（无尽模型，[设计 49](#49-infinite-no-rounds)）：棋盘卡死（无处可落）不结束游戏，用消除道具清块脱困（[49·§3.1](#49-infinite-no-rounds::cleartool)）；体力归零不结束游戏，靠时基恢复（含离线）回升（[49·§3.2](#49-infinite-no-rounds::regen)）。订单无限取用、无通关终点；整盘续存，退出重进从原状态继续（[设计 14](#14-save-system)）。
...
| 时基恢复 | +1 / 120s | 无条件、纯时间驱动、含离线（[设计 49·§3.2](#49-infinite-no-rounds::regen)）；按现实时间跨会话恢复，仅回到软上限。是无尽的最终保险——体力归零靠它迟早回升 |
| 体力耗尽兜底 | 不结束游戏（无尽模型） | 体力 &lt; 落子消耗 时暂时落不起子；靠时基恢复（含离线）回升（[设计 49·§3.2](#49-infinite-no-rounds::regen)），不弹任何结束面板 |
...
### 风险 2 · 体力上限与会话落子节奏的匹配 {#risk2}

**问题**：体力既要在早期不卡手，又要让玩家感到它是真实约束；上限/回速不能照抄浪漫餐厅（那是纯挂机节奏，本作是<mark class="y">活跃落子节奏</mark>）。

> [!TIP]
> **方案（按本作落子节奏重算，给出预算数学）：**
>
> - **预算下界**：无任何消除、无订单完成的最坏情况，起始 20 体力 = 20 次落子后体力归零——足够玩家撞出几次消除并完成首单（归零不结束游戏，靠时基恢复回升，[设计 49](#49-infinite-no-rounds)）。要求 **起始体力 ≥ 完成首单所需落子数的期望**，否则首单前就卡节奏。
> - **可持续点**：一个约 3 块的批次配 1 次单行消除 ≈ 净 -2 体力；每 ~6–8 次落子完成 1 单 → +8 体力 → **熟练玩家体力净正**——正中「技巧驱动：会消除→玩得久」的定位。无尽模型下体力不再「终结一局」（无 GameOver），净正只是让活跃节奏不被体力卡住。
> - **取舍定档**：软上限取 **30**——高到早期不饿死、低到体力是被感知的资源。公式与各常量全部可调，据此重配（核心不变量：**起始体力 ≥ 完成首单的落子数**，否则首单前就卡节奏）。

### 风险 3 · 落错子的兜底（无广告） {#risk3}

**问题**：落子耗体力且可能把棋盘逼进卡死（无处可落）；一次失误代价大，而项目红线<mark class="y">禁广告复活</mark>。需要不依赖变现的解围阀。

> [!TIP]
> **方案 · 限次免费悔棋（撤销失误）+ 消除道具（卡死脱困）：**
>
> - **悔棋（撤销最近失误）**：3 次免费悔棋（无广告、无内购，纯次数限制，契合去变现）。**范围**：仅撤销**最后一次落子**（单步）。落子前对受影响状态打快照，六项：当前棋盘占位、棋盘上的元素分布、体力值、合成区库存、订单进度、待投放元素队列。悔棋 = 整体回滚到该快照 + 把方块退回待选槽 + 退回该次落子扣的体力。悔棋是**纯内存机制**，不进盘（[设计 14·§2.2](#14-save-system::snapshot-vs-disk)）。
> - **更轻的备选**：仅当该次落子**未触发消除**（纯失误落子）时允许单步撤销，省去回滚消除/合成/订单的复杂度。若全量单步快照成本高，可先落地此备选版，全量快照列为增强。
> - **悔棋次数耗尽后**：无尽模型下不进死局判定、不弹结束面板（[设计 49](#49-infinite-no-rounds)）。真正卡死（无处可落）由**消除道具**清块脱困（[设计 49·§3.1](#49-infinite-no-rounds::cleartool)，代价体力、无限可用），与悔棋分工：悔棋撤销「刚才那一手」、消除道具脱困「整个棋盘卡死」。
...
| 订单 | 订单池 | 无限循环（无通关单数，[设计 49](#49-infinite-no-rounds)） |
| 体力 | 起始体力 | 20 |
| 体力 | 软上限（奖励可溢出） | 30 |
| 体力 | 落子消耗 | 1 / 块 |
| 体力 | 消除返还 | 消除行列数 |
| 体力 | 时基恢复 | +1 / 120s（无条件 / 含离线，[设计 49·§3.2](#49-infinite-no-rounds::regen)） |
| 体力 | 消除道具 cost（卡死脱困） | 25（≤ 软上限，无限可用，[设计 49·§3.1](#49-infinite-no-rounds::cleartool)） |
| 体力 | 耗尽兜底 | 不结束游戏（无尽模型）；靠时基恢复回升 |
| 元素生成 （得分驱动） | 每元素折算分数 | 200（每 200 分折 1 元素） |
| 元素生成 （得分驱动） | 单次消除保底元素数 | 1（任何消除保底产 1） |
| 元素生成 （得分驱动） | 单次消除封顶元素数 | 4 |
| 元素生成 （得分驱动） | 待投放队列上限 | 12 |
| 兜底 | 悔棋次数（撤销最近落子） | 3（纯内存机制，不进盘） |
...
| 体力卡点 | 卡「生成器产出」 | **改** → 卡「落子」；且落子不保证消除（产出有方差），棋盘可能卡死——卡死由消除道具脱困（[设计 49](#49-infinite-no-rounds)），不结束游戏 |
| 体力数值 | 上限 100、2 分钟 1 点（跨会话挂机节奏） | **重算** → 软上限 30、活跃节奏；时基恢复（含离线）实装（[设计 49·§3.2](#49-infinite-no-rounds::regen)），主补给来自订单奖励 |
...
本切片以独立窗口承载，复用 Classic 的棋盘 / 拖拽 / 落子流程，在落子结算流程中插入：落子扣体力 → 消除返体力 → 被清元素入合成区（自动合并）→ 刷新订单 / 合成区 / 体力显示 → 检查订单可交付 → 订单交付即刷新下一单（无限）。无尽模型（[设计 49](#49-infinite-no-rounds)）下无结束面板：卡死时消除道具按钮可用脱困、体力归零靠时基恢复回升。界面零美术（glyph + 纯色）：订单卡显示「元素 glyph + 等级 + 数量 + 交付按钮（满足点亮）」、合成区显示各类型当前等级、体力条显示当前/上限、消除道具按钮（体力够时可用）。
...
- 体力系统：落子扣、消除返、订单补、时基恢复（含离线，[设计 49](#49-infinite-no-rounds)）、耗尽不结束游戏。
- 合成区：消除元素入区 → 自动两两合并升级（封顶 Lv3）+ 显示。
- 订单系统：双订单、手动交付（消耗合成物 + 发奖）、无限刷新、锯齿波（循环池实现）。
- 元素生成：消除得分驱动数量（确定性映射）+ 需求拉动类型 + 队列上限。
- 落错兜底：限次免费悔棋（单步快照回滚，纯内存机制）。
- 卡死脱困：消除道具（[设计 49·§3.1](#49-infinite-no-rounds::cleartool)）。
- 无尽循环（无通关 / 无 GameOver）+ 整盘续存（[设计 14](#14-save-system)）。
...
| 关卡框架 / 多关 / Luban 接表 | Demo 切片边界，配置硬编码（注：时基恢复 + 离线结算已由[设计 49](#49-infinite-no-rounds)实装、整盘存档已由[设计 14](#14-save-system)实装，不再 out-of-scope） |
...
| 落错子 / 棋盘卡死且无广告复活 | 限次免费悔棋（撤销最近失误，3 次，单步快照）+ 消除道具（卡死脱困，[设计 49](#49-infinite-no-rounds)）（[§四·3](#09-merge-order-energy::risk3)） |
...
- [11 · 核心玩法补全(完整设计)](#11-core-loop-completion)
