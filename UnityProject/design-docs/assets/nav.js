...
(function () {
  ...
  const GROUPS = [
    ...
    { side: '切片设计', card: '切片设计(活跃)', docs: [
      ...
      { href: '14-save-system.html', side: '14 · 跨会话存档', tag: '系统 · 持久化', title: '14 · 跨会话磁盘存档(整盘续存)',
        desc: '无尽模型(见 49)下取消「局」与重开,存档从「只存元层」升为整盘续存:元层进度(虔诚币/神庙/经验·守护者等级/灵力/盲盒/女神/今日祈愿)+ 局内态(棋盘/手牌/合成区/进行中订单/体力/待投放队列/连消链/全清武装)都进盘,退出重进从原状态继续(断点续玩=必做)。只有纯内存瞬态(悔棋栈)不进盘。体力进盘 + 加载时按真实时差补算离线时基恢复(联动 49 §3.2;离线恢复=脱困保险非养体力漏洞,反转旧 O3 论据)。完成单数/交付分无通关后作生涯累计统计进盘。序列化层同步可单测、磁盘 IO 走异步外壳;含 version=2 迁移(旧档元层保留+局内态走缺省)、缺字段保底、跨天重置、离线体力补算、与悔棋内存机制分层、18 条验收点。' },
      { href: '29-gameplay-fusion.html', side: '29 · 玩法融合', tag: '设计 · 顶层裁决', title: '29 · 玩法融合 · 经典吸收进合成订单',
        desc: '把两个并存入口(经典无尽 01 / 合成订单 11)融成一套:经典核心完全吸收为底层引擎(DDA/计分/方块库/连消钩子下沉),合成订单的体力/合成/订单/宝箱/女神/神庙是完整经济,移除纯无尽独立入口、保留体力预算。核心是 8 冲突点逐条裁决(入口/结束条件/计分语义/发牌/连消钩子点亮/方块库/存档/DDA 适配),每条给经典现状×合成订单现状×融合后取定+落点。基于代码现状(已远超旧设计稿快照:ClearSettlement/HandGenerationArbiter 已落地)逐条核实,标注现状/本篇目标。含经典保留(6项)/被覆盖移除(3项)清单、代码层融合落点(主菜单单入口/两窗合一/三隐患:Combo·Score 角色归一·dynamicWeight 跨模式残留·DDA 无分压适配/存档合并)、四项范围开关默认+备选、拍板记录(2026-06-16)与验收点。<b>注:旧三出口结束模型(通关/软/硬 GameOver)已被无局·无尽模型覆盖,见 49。</b>仅策划阶段不改码。' },
      { href: '49-infinite-no-rounds.html', side: '49 · 无局 · 无尽核心模型', tag: '设计 · 核心模型修正', title: '49 · 无局 · 无尽核心模型',
        desc: '合成订单融合玩法从「分场次/有通关/可 GameOver」改为单条持续运营的无尽循环:无「局」概念、订单无限、无任何 GameOver、局内态(棋盘/手牌/合成区/进行中订单/体力)整盘跨会话续存。两条兜底保证真·无尽:① 体力时基恢复(无条件/纯时间驱动/含离线累计/封顶软上限),离线恢复=预期脱困路径非「养体力」漏洞(反转旧设计14 O3 论据);② 消除道具(卡死时主动清一行一列、代价体力 cost≤软上限默认25、无限可用只 gate 体力、绝不做有限消耗品)。含脱困证明(卡死+0体力最坏点走通+三条硬约束)、配平取向(防「卡了就清/挂机等体力」无脑循环+全清奖不被消除道具白嫖)、待拍板4档(脱困形态/cost/恢复速率/完成单数定位)、文档自检A+落地核验B 验收。覆盖旧结束模型,同步重写 01/09/11/29/14。' },
      { href: '50-block-skin-switch.html', side: '50 · 方块皮肤切换(全清触发)', tag: '设计 · 棋盘表现', title: '50 · 方块皮肤切换(全清触发单色换皮)',
        desc: '棋盘方块两套皮肤的切换设计:初始彩色(每种方块类型各自固定配色)→ 首次「全清(All-clear / perfect clear,整盘被清空那一刻)」整盘转单色(全盘所有方块不分类型统一显示从单色资源随机选中的同一张)→ 此后每次全清换一张单色、排除当前在用那张(防连续重复)。明确「全清」≠ DDA「清屏窗口」(01/02/07 的分数难度窗,语义不同切勿挂错):本功能特指棋盘整盘被清空事件,复用 11·§5.4/§5.5 的全清判定。皮肤态(彩色/单色 + 当前单色 sprite 标识)按无尽模型(49)作局内态跨会话续存(对齐 14 存档边界),退出重进保持。含状态机、单色随机选取规则(从真实存在的 sprite 编号集随机、非 1..N 连续区间)、防连续重复边界、与女神「换背景」(纯背景轴,非方块轴)的区分、美术规格(源目录/图集名/sprite 命名)、可单测的状态机/防重/续存验收 + 需手验的表现层验收分列。仅策划阶段,不写代码符号。' },
    ]},
    ...
    { side: '全栈 / 服务端', card: '全栈 / 服务端', docs: [
      ...
      { href: '45-player-attr-ledger-query.html', side: '45 · 玩家属性 ledger 查询协议(Tier 2 第 3 子单 · server 段 + 客户端协议生成物)', tag: '全栈 · ledger 查询通路 server 段', title: '45 · 玩家属性 ledger 查询协议(Tier 2 第 3 子单 · server 段 + 客户端协议生成物)',
        desc: 'Tier 2 玩家属性权威体系 server 段查询通路刀:兑现 44 §6.2 「客户端我的流水 UI + 拉流水 RPC」接口余量声明的协议契约 + 服务端 handler + 客户端协议生成物三件交付。新协议 C2G_QueryAttrLedger(kind 可选 / sinceTs 可选默认 0 / limit 必填,服务端钳制上限 100) + G2C_QueryAttrLedgerResponse(resultCode + entries[] + hasMore);handler 按 (account, timestamp DESC) 主索引取前 limit 条(命中 44 §3.2 ix_account_ts_desc),支持 kind 过滤 + sinceTs 滑动窗口(ts > sinceTs 增量同步语义);entry 行白名单 7 字段(timestamp / kind / balanceBefore / balanceAfter / delta / source 整数枚举 / reasonRaw),不暴露内部 ObjectId / SchemaVersion / account(沿 32 / 33 不暴露内部 id 范式 + conventions 不投机暴露未来用不上的接口)。错误码集 3 个(Success / InvalidRequest 含 kind 未知 / sinceTs 负数 / ServiceUnavailable 含 MongoDB 不可达)。客户端业务接入(RemoteAttrLedgerService 沿 22 RankService 远程源同范式) + 我的流水 UI 投放(沿 28 排行榜窗 art 受限范式) + source 整数 → 文本映射 = 46 客户端段已落(客户端段第 4 子单收口)。核心架构决策:① 协议字段语义 sinceTs 是「ts > sinceTs」上界过滤增量同步,不是翻旧页 cursor(翻旧页留 Tier 2+ 扩 untilTs / offset / cursor 任一);② 身份从会话取(handler 忽略请求中任何 account 字段),协议字段层面不预留 account(防拉他人 ledger,沿 37 / 38 / 44 同范式);③ limit 钳制 [0, 100] 不报错(降级语义,防 DOS:limit=999999999 实际返 100 + hasMore=true);④ kind / sinceTs 非法值校验(kind 整数 0/1/2/3 之外返 InvalidRequest,sinceTs 负数返 InvalidRequest);⑤ handler 只读 ledger 集合(永不 InsertOne / UpdateOne / DeleteOne / FindOneAndUpdate / FindOneAndDelete,沿 44 §5.4 追加式 invariant,Code Review 必拦);⑥ MongoDB 异常 catch 返 ServiceUnavailable 不抛(沿 32 / 44 不可达不抛口径);⑦ 响应字段裁剪 7 字段白名单(Code Review 拦其他字段);⑧ source 整数 → 文本映射留客户端段下一刀做(协议层只传整数,文本由 i18n 或硬编码 switch);⑨ 服务端段 + 客户端协议生成物分两件交付,客户端业务接入与 UI 投放留下一刀(沿 32 邮件 server 段先行 + 客户端段后续同范式,5d0d28e4 邮件协议生成物 + d52dbdd6 排行榜协议生成物先例)。同任务内同步重写(sweep 闭合):① 44 §6.2 接口余量表「客户端我的流水 UI + 拉流水 RPC」演进列覆盖式重写为「45 已交付服务端段 + 协议生成物;客户端业务接入 + UI 投放留 Tier 2+ 客户端段后续刀」;② 44 §一 切分表「拉流水 / 查历史 / 我的流水 UI」归属列从「不做 · Tier 2+」覆盖式重写为「协议契约 + 服务端 handler + 客户端协议生成物 = 服务端 · 45 已交付」;③ 44 §5.4 不守列表「客户端可查 ledger」行覆盖式重写指向 45;④ 44 §7.2 BLOCKED 列表「客户端我的流水 UI / 拉流水 RPC」行覆盖式重写指向 45。验收 SV1-SV18(服务端编译 + 源生成器产物 / handler 注册 / Success 返空 / Success 返非空 / limit 钳制上限 100 / limit=0 返空 / kind 过滤生效 / kind 未知 InvalidRequest / sinceTs 滑动窗口 / sinceTs 负数 InvalidRequest / 身份从会话取 / MongoDB 不可达 ServiceUnavailable / 索引命中 IXSCAN ix_account_ts_desc / handler 仅只读 / 响应字段裁剪 7 字段 / 既有服务端段零回归 / hasMore 语义边界 / Code Review 重点核 10 项)+ CV1-CV3(客户端协议生成物存在 + 编译过 / git diff 仅 Assets/GameProto/ / 既有客户端段零回归)+ E1(真往返双端共识)。真往返查 ledger 类 SV 依赖本机 MongoDB,不可达列 BLOCKED 非 FAIL。仅策划阶段,不写代码符号。' },
      { href: '47-activity-cumulative.html', side: '47 · Cumulative 节律落地 + 累计游戏 N 局样例(Tier 4 第 4 子单 · server)', tag: '全栈 · Cumulative 节律首次实做', title: '47 · Cumulative 节律落地 + 累计游戏 N 局样例(Tier 4 第 4 子单 · server)',
        desc: 'Tier 4 活动系统第 4 子单 server 段:把 39 §3.5 声明的 Cumulative 节律接缝(ActivityProgressService.Increment(account, activityId, delta),留 O3 架构挖坑)首次真实做。加客户端可触发的归一 RPC C2G_ActivityIncrement(activityId, delta) + handler 校验 5 项(身份从会话取 / activityId 配置存在 / 配置 type=Cumulative / delta>0 / delta≤10000 钳制)+ 错误码集 4 档(Success/InvalidRequest/NotCumulative/ServiceUnavailable)+ 通用 ActivityProgressService.Increment service 实做(沿 39 §3.4 发奖编排一字未改,只把「+counter 起点」从「登录触发」换为「客户端业务推 + 服务端进程内调」)+ 首套 type=Cumulative 样例活动(累计游戏 100 局,OneShot 永发一次,activity_id=5)+ AuthoritativeDefs 注册新行 + 客户端协议生成物。本子单纯 server only,客户端业务接入(GameOver hook 调 RPC)+ UI 反馈 + 离线缓存 pending delta 策略留下一子单。审计后调查结论:① 简报「ActivityProgressService.Increment API 形态(已有接缝在 39 §3.5,本子单兑现实装)」与现状一致(39 §3.5 Cumulative 行声明接缝 + 留 O3「架构挖接缝,实现待业务接入」);② 简报「新 RPC 协议字段 + 错误码集,或复用其它 RPC(典型扩 C2G_GameEnd 携活动达标元数据)」取新建专用 RPC 而非复用 GameEnd(工程 grep 无 C2G_GameEnd 协议 + 即便有也违协议关注点单一 + Cumulative 类触发源多元化各扩 RPC 不可持续,沿 30/31 归一专用 RPC 范式);③ 简报「handler 校验项:身份从会话取/activityId 必须 type=Cumulative/delta 必须正数 + 上限/防刷」核为 5 项校验链(身份+activityId 存在+type=Cumulative+delta>0+delta≤10000 钳制)+ 4 档错误码归一(沿 32/33/45);④ 简报「Increment 后是否走 §3.4 发奖编排」取完全沿用(共用 39 §3.4 已实做的「判达标+抢占+SendMailTo」流程);⑤ 简报「第 1 套样例活动:累计游戏 N 局,触发源 = 客户端 GameOver,客户端业务接入留下一刀」严守(server-test mock 客户端 mock 发包验完整 server 通路 SV2-SV10,无需客户端 GameOver 真触发)。范围严守:本子单 server only + 客户端协议生成物 + 编译过;不动 39 ActivityDef/activity_progress schema/§3.4 发奖编排/§3.5 Login 触发节律/32 SendMailTo 签名/既有六全栈 + 35/36/37/38/40/41/43/44/45/46 任何代码;不做 Daily/Weekly Cumulative 样例验证(service 须前瞻支持但本子单只 OneShot 样例验过);不做其它 4 套 Cumulative 类活动(累计交付/累计消费/累计获得/累计签到,留运营后续逐套刀加 1 行配置 + 客户端业务接入);不做客户端 GameOver hook 接入 + UI 反馈 + 离线缓存策略(留下一子单)。核心架构决策:① 协议形态:新建专用 RPC C2G_ActivityIncrement(归一)而非复用/扩 C2G_GameEnd(违协议关注点单一 + Cumulative 类触发源多元化不可持续);② handler `type=Cumulative` 强制校验(反作弊硬约束,防客户端推 type=Login 活动旁路登录节律);③ handler 错误码 4 档归一(Success/InvalidRequest/NotCumulative/ServiceUnavailable,NotCumulative 单列便客户端差异化提示 + 反作弊日志统计);④ 单次 delta 上限 10000(防恶意推爆,钳制不报错让客户端从 response.currentCounter 自查);⑤ 样例 activity_id=5 沿 39/40/43 占位段递增,target=100 OneShot 中期目标长期成就;⑥ mail_def=7005 + giftrandom=5005 可选加(可砍 mail_def=0 + reward 复用 40 6101 EVENT 礼包或 43 5003 钻石礼包);⑦ Cumulative 节律触发源本子单只支撑客户端 GameOver 一类(下一刀接,客户端业务接入留下一子单);⑧ service 须前瞻支持 Daily/Weekly Cumulative(跨周期 counter 清零沿 43 §3.2 范式);⑨ 服务端进程内 API Increment vs handler 双路统一同一逻辑路径(DRY + 单一权威);⑩ type 枚举档加 Cumulative=2(若 39 第 1 子单已加则零增);⑪ counter 类型 long(沿 39 §3.2);⑫ MongoDB `$inc` 原子累加(非「读++delta+写」沿 33 §3.2);⑬ 服务端不可达不本地放行(沿 30 §3,客户端段下一刀决定缓存策略);⑭ 守 39/40/43 + 既有六全栈 + 32 SendMailTo 签名 + 35-46 所有不变量。整局走查 12 类崩法(handler 漏校验 type / delta 上限漏钳 / counter 累加并发漏 delta / 判达标+写已发周期键非原子 / SendMailTo 中途崩漏发 / Daily/Weekly Cumulative counter 跨周期不清零 / 服务端不可达客户端本地放行 / 协议字段缺失格式错 / handler 校验顺序错泄露信息差 / 客户端伪造 account / service 层 type 校验漏 / 抢占在 SendMailTo 之后致超发)+ 各类对策。同任务内同步重写(sweep 闭合):① 39 §3.5 节律表 Cumulative 行状态从「留 O3」覆盖式重写为「第 4 子单已兑现」;② 39 §3.5 节律旁注追加「+ 1 套 type=Cumulative 活动」;③ 39 §3.1 配置表 type 字段旁注 Cumulative 行状态从「留 O3」覆盖式重写为「第 4 子单已兑现」;④ 39 §七 O2 状态行覆盖式重写;⑤ 39 §七 O3 Cumulative 部分覆盖式重写;⑥ 39 §八 BLOCKED 列表 Cumulative 部分覆盖式重写;⑦ nav.js GROUPS 加 47 入口;⑧ index.html ?v 递增防缓存。验收主验:SV1 配置同源导出(activity.xlsx 加 1 行 activity_id=5 + __enums__.xlsx 活动 type 含 Cumulative + 可选 mail.xlsx 7005 + giftrandom.xlsx 5005);SV2 ActivityProgressService.Increment service 实装(沿 §3.5 行为契约);SV3 C2G_ActivityIncrement RPC handler 实装(身份从会话取 + 5 项校验链);SV4 累计 100 局达标 + 发邮件 + 后续无重发(targetReached 第 100 次 true);SV5 type 校验拒不符活动(推 type=Login 活动返 NotCumulative + 文档零改动);SV6 delta 负数/零/缺失字段/activityId 不存在拒(返 InvalidRequest + 文档零改动);SV7 delta 上限钳制(50000 钳到 10000 + 不抛 + Success);SV8 同账号并发原子幂等(`$inc` + FindOneAndUpdate 双层原子,counter 最终正确 + 仅 1 封邮件);SV9 跨会话/重启幂等(OneShot 永发停);SV10 真往返(起服 + 100 次 RPC + 验文档 + 邮箱 + 领奖);SV11 既有 39/40/43 PASS 行为零回归(活动 1/2/3/4 行为不变 + activity_progress/activity.xlsx schema 零字段加 + 32 SendMailTo 签名零改 + 39 §3.4 发奖编排零改 + 既有全栈不破);SV12 Code Review(身份从会话取 + handler+service 双层 type 校验 + Increment 统一逻辑路径 + counter `$inc` 原子 + 跨周期 counter 清零 + 校验顺序先 activityId 后 type + delta 上限钳制 + 抢占必在 SendMailTo 之前 + 发奖必经 32 §3.5 入口 + 无新集合/schema 字段加/既有协议改);PVC1 客户端协议生成物加载(Unity 编译过 + 字段集与协议契约一致);PVC2 客户端业务接入零 diff(留下一子单);E1 全栈真往返(累计 100 局 → 邮件领奖 → 若 reward 含货币 → 37 PropertyChangeRequest → 38 G2C_PropertyDeltaPush → 42 tarot HUD 见变化 + 44/45/46 ledger 写一行);E2 跨会话(重启 + 同账号再推不重发);E3 全栈零回归。真往返写库依赖本机 MongoDB,不可达列 BLOCKED 非 FAIL;Tier 4 第 4 子单交付后 Cumulative 节律从「架构挖坑」升「真实可用」+ 1 套样例验通路,后续运营加同类 Cumulative 活动只需配 1 行 + 客户端业务系统调一次 RPC(零代码改 = 沿 43 同范式应用在 Cumulative 上)。仅策划阶段,不写代码符号。' },
      { href: '48-activity-cumulative-client.html', side: '48 · Cumulative 客户端 GameOver hook + RemoteActivityService(Tier 4 第 5 子单 · 客户端段)', tag: '全栈 · Cumulative 节律客户端段(收口)', title: '48 · Cumulative 客户端 GameOver hook + RemoteActivityService(Tier 4 第 5 子单 · 客户端段)',
        desc: 'Tier 4 活动系统第 5 子单 · 客户端段(收口 Cumulative 节律):把 47 已交付的服务端归一 RPC C2G_ActivityIncrement 接进客户端业务,新建 RemoteActivityService 远程服务(沿 46 RemoteAttrLedgerService + 32 RemoteMailService 编排范式,挂入 GameContext.Activity)+ IActivityIncrementSource 接缝 + RemoteActivityIncrementSource 生产实现(经 38 §五 IRpcGateway 范式,业务层不直引 Fantasy.*)+ POCO ActivityIncrementResult(3 字段映射 47 §3.2:Code/CurrentCounter/TargetReached)+ ActivityIncrementCode 枚举(5 档:Success/InvalidRequest/NotCumulative/ServiceUnavailable/NetworkDown,沿 46 AttrLedgerQueryCode 范式 + NotCumulative 补)+ 三处 GameOver 出口 fire-and-forget hook(Classic GameWindow.TriggerGameOver / MergeOrder MergeOrderWindow.TriggerGameOver 双 GameOver 路径共用 / MergeOrder TriggerWin 通关也算一局)调 IncrementAsync(5, 1)。本子单纯客户端业务 hook 接入,无 UI 投放、无 toast 弹窗、无离线缓存——沿 EVENT 解锁同范式服务端达标自动投活动邮件,客户端从邮件领奖。审计后调查结论:① 简报「GameOver hook 落点 GameWindow.TriggerGameOver vs BlockGameState 的 GameOver 信号 vs 主玩法 OnUpdate 检测」grep 工程现状定 GameWindow.TriggerGameOver(Classic)+ MergeOrderWindow.TriggerGameOver/TriggerWin(MergeOrder)三处出口;② 简报「同时记 Tarot 模式 vs 只 Classic 计数 vs 双计数」取 Tarot/Classic 共计(沿 26 tarot_mode HUD = Classic GameWindow 再主题,GameWindow.TriggerGameOver 调一次 = 1 局,与玩法子模式正交);③ 简报「离线策略不缓存 vs 本地 PendingIncrements 队列」取不缓存(沿 30/32/46 同口径不本地放行,与服务端权威 counter 抵触,本地放行 = 服务端复连后丢失推送 = 审计裂缝 = 不可降级;真做替代留 Tier 4+ 需服务端补 idempotency key 字段违 47 协议不改);④ 简报「UI 反馈无 vs 最简 toast vs 进度条」取无(沿 EVENT 同范式服务端达标自动投活动邮件 + GameOver 焦点不被抢 + 「玩 99/100 还差 1 局」类文案是变现诱导 + 复杂度大)。范围严守:本子单只接 Classic + MergeOrder 三出口 fire-and-forget + service/source 编排;不做我的活动进度 UI(留 Tier 4+ 运营加多套 Cumulative 活动后做集中展示窗 + 需服务端加查询 counter 协议)+ 不做多套 Cumulative 接入(累计交付/累计消费/累计获得/累计签到,留运营后续逐套刀,每套加 1 行 activity.xlsx + 客户端业务出口加 IncrementAsync(newId, 1) 调用即可)+ 不做 Tarot/Classic 分别计数 + 不做离线缓存 + 不做达标 toast/进度条/「我的活动进度」窗。核心架构决策:① 三处 GameOver 出口等价计数(Classic GameOver + MergeOrder 软/硬 GameOver + MergeOrder 通关,玩家视角都是「玩了一局」,通关也算否则「玩得越好越没奖」反直觉);② Tarot/Classic 复用同一 GameWindow 实例不分别计数(沿 26 决策);③ fire-and-forget 不阻塞 GameOver 流程(不 await + 不订阅响应 + 错误码归一日志即止);④ 离线/服务不可达不本地缓存 pending delta 直接丢弃(沿 30/32/46 不本地放行);⑤ 无任何活动相关 UI 反馈(沿 EVENT 同范式);⑥ 错误码客户端处理 5 档分级(Success/Info,InvalidRequest+NotCumulative/Error 代表 bug,ServiceUnavailable+NetworkDown/Warning 是环境抖动);⑦ HotFix 业务层不直引 Fantasy.*(沿 38 §五 IRpcGateway 范式);⑧ GameContext.Activity 启动期 GameApp.StartGameLogic 创建实例 + 注入生产 source(沿 38/46/32 同范式);⑨ activityId/delta 在 hook 处硬编码(本子单 5/1 单点),未来扩 O7 常量集中 ActivityIds;⑩ 既有防重标记(_gameOverTriggered/_finished)守 hook 每局只调一次,hook 调用在防重之后;⑪ 同账号并发两次 IncrementAsync(玩家短时间连玩两局)沿 47 SV8 已验原子幂等(`$inc` 文档级原子);⑫ MergeOrder 精力耗尽 + GAME OVER 双 GameOver 路径共用同一 TriggerGameOver(title) 函数,hook 加一处两路覆盖;⑬ 不动 Classic/MergeOrder 玩法循环核心(PlaceAndResolve/落子/ghost/拖拽零改);⑭ 守 47 协议/handler/service/配置 + 32 SendMailTo + 38/42/46 已落客户端段 + Atlas/Sheet 图集所有不变量。整局走查 13 类崩法(同一局 GameOver 触发两次 / Tarot 与 Classic 双推 / 通关被漏接 / MergeOrder 双 GameOver 路径漏接一路 / GameContext.Activity 启动期未挂 NRE / 网络断 Session 未建 / 服务端 MongoDB 抖 / 玩家关 App 响应未回 / 玩家连开两局并发 / 长时间离线大量游戏 / 客户端传错 activityId / delta 异常 / 协议字段缺 / `_ = IncrementAsync` 漏 `.Forget()` warning)+ 各类对策。同任务内同步重写(sweep 闭合):① 47 §一立项框「客户端业务接入留下一刀」+ §3.6 跑通示例「下一刀 GameOver hook 接」+ §四 服务异常表客户端本地放行决策 + §五崩法表对应行 + §七 O6 + §八 PVC2 + §八 BLOCKED 列表 + §九风险表「范围溢出做客户端 GameOver hook 接入」 + §相关文档 一律覆盖式重写指向「48 已交付」;② nav.js GROUPS 加 48 入口;③ index.html ?v 递增防缓存。验收主验:PV1 Unity 编译过;PV2 既有客户端段零回归(Classic/MergeOrder/GameOverWindow/MergeOrderWinWindow/38/42/25/27/46/32/22 全 pass);PV3 Classic GameOver 触发 hook + 日志可见;PV4 MergeOrder 软/硬 GameOver 触发 hook + 日志可见;PV5 MergeOrder 通关触发 hook + 日志可见;PV6 同一局 GameOver hook 只调一次(防重);PV7 Tarot/Classic 共计不分别计数;PV8 GameContext.Activity 启动期挂入 + null-safe;PV9 fire-and-forget 错误码归一 + 各级日志;PV10 fire-and-forget 不阻塞 GameOver 流程(延迟 5s 响应也不影响);PV11 ServiceUnavailable 兜底 + 无 UI 反馈;PV12 NetworkDown 兜底 + 无 UI 反馈;PV13 离线不缓存 pending delta(Code Review 反证);PV14 HotFix 业务层不直引 Fantasy.*;PV15 Code Review 9 项(`.Forget()` 显式 / 防重之后调 / null-safe / catch 转 NetworkDown / 无本地状态 / 硬编码 5/1 / GameContext 启动挂入 / 不动玩法核心 / 不动 UI 节点)+ E1 真往返 Classic 玩一局 counter +1;E2 真往返累计 100 局达标 + 邮件可见(若 reward 含货币联动 38/42/46);E3 真往返第 101 局服务端不重发;E4 真往返 MergeOrder 三路 hook 服务端均收到;E5 真往返 Tarot/Classic 共享 counter;E6 真往返断服 GameOver 不报错 + counter 不动 + 玩家无感;E7 真往返 fire-and-forget 不延迟 GameOver UI(秒级弹窗)。真往返 E 类依赖本机 MongoDB,不可达列 BLOCKED 非 FAIL(沿 47 §八 + 46 §7.3 同口径)。仅策划阶段,不写代码符号。' },
      { href: '46-player-attr-ledger-client.html', side: '46 · 我的流水 UI + RemoteAttrLedgerService(Tier 2 第 4 子单 · 客户端段)', tag: '全栈 · ledger 查询客户端段(收口)', title: '46 · 我的流水 UI + RemoteAttrLedgerService(Tier 2 第 4 子单 · 客户端段)',
        desc: 'Tier 2 玩家属性权威体系客户端段收口刀:兑现 45 §6.2 「客户端业务接入 + 我的流水 UI + source 文本映射」三项接口余量,新建数据源接缝 IAttrLedgerSource(沿 32 IRemoteMailSource 范式)+ 生产实现 RemoteAttrLedgerSource(经 FantasyNetwork.Session 发 45 已交付的 C2G_QueryAttrLedger)+ 客户端服务 RemoteAttrLedgerService(沿 32 RemoteMailService 编排范式,挂入 GameContext.AttrLedger)+ POCO AttrLedgerEntry / AttrLedgerPage(失败时空集合非 null)+ 客户端错误码 AttrLedgerQueryCode(4 档:Success / InvalidRequest / ServiceUnavailable / NetworkDown,沿 38 ChangeReject 加 NetworkDown 区分服务端不可用与客户端断网)+ source 整数 → 中文文案硬编码 switch(10 档覆盖 44 §3.3:Unknown 其他 / ChangeNameSpend 改名扣钻 / MailClaim 邮件领奖 / RedeemCode 兑换码 / RankSettleReward 排行榜奖励 / ActivityReward 活动奖励 / GameplayConsume 玩法消费 / ShopPurchase 商店购买 / AdminGrant 管理员发放 / Refund 退款,default 「其他」降级)+ 时间显示混合范式(< 1min 「刚刚」/ < 1h「N 分钟前」/ < 24h 「N 小时前」/ < 7d 「N 天前」/ ≥ 7d 本地时区 yyyy-MM-dd HH:mm,FormatRelativeTime(ts, now) now 入参化便单测)+ 独立窗 PlayerAttrLedgerWindow(沿 28 排行榜窗范式,过滤栏四档全/金/钻/体 tab + 列表行四列时间/属性图标/delta/source/余额变化 + 加载更多按钮 + 关闭按钮 + 状态栏错误兜底)+ 入口按钮挂 PlayerInfoWindow 改名面板下方(独立窗而非 tab/HUD 长按,§4.2 决策)+ 属性图标复用 42 HUD 占位(Coin → gemstone / Diamond → gemstone2 / Stamina → potion,不新增切图)。核心架构决策:① 客户端不持本地 ledger 副本(服务端独占审计完整性,沿 44 §5.4);② 离线/服务不可达不本地放行(空列表 + 文案,沿 32 邮件领奖断服不本地);③ source 文案硬编码(去变现 i18n 暂无,O5);④ 加载更多按钮置灰提示(协议 sinceTs 是「ts > sinceTs」上界过滤增量同步语义、非翻旧页,翻旧页需 Tier 2+ 协议加 untilTs 字段,本子单首屏 50 条不接续翻);⑤ 切 tab 取消旧请求 _currentFetchToken(沿 22 RankService 范式)+ Window 销毁解订阅 + _disposed 标记跳过 UI 更新(防 dispose 后 NRE);⑥ 入口按钮 FindChildComponent null-safe(节点缺失不抛,美术补图后即用);⑦ HotFix 业务层不引用 Fantasy.* 命名空间(网络层薄壳沿 38 §五 IRpcGateway 范式)。同任务内同步重写(sweep 闭合):① 45 §6.2 / §一切分表 / §5.4 / §7.4 四处客户端业务接入 + UI 投放 + source 文本映射相关行覆盖式重写为「46 已交付」;② 44 §一切分表 / §6.2 接口余量表对应行同步「客户端 · 46 已交付」;③ 25 §三节点树 + §六交付列表加「我的流水入口按钮 m_btn_LedgerEntry 节点占位」(美术 Tier 2+ 补图,本子单不阻塞)。验收 PV1-PV15(Unity 编译 / 既有客户端段零回归 / 空列表 / 非空列表 timestamp DESC + 四列展示 / kind 过滤切 tab 重拉 / source 未知值不抛显「其他」 / 时间显示边界 EditMode 单测 / 流水窗与 38 PlayerAttrService.IsReady 独立 / ServiceUnavailable 兜底 / NetworkDown 兜底 / 切窗切 tab 期间响应回来不 NRE / 入口按钮 null-safe / 加载更多按钮置灰文案诚实 / Code Review 8 项 / Fantasy PlayerAttrLedgerDoc.Kind 注释 Coin=1/Diamond=2/Stamina=3 与实际 PropertyType Coin=0/Diamond=1/Stamina=2 不符 carry-forward 已清)+ E1-E4 真往返(改名扣钻后流水可见 / 登录奖到账后流水可见若 32→37 接线已落 / kind 过滤服务端只返过滤后行 / 服务不可达兜底)。真往返查 ledger 类 PV 依赖本机 MongoDB,不可达列 BLOCKED 非 FAIL(沿 45 §7.4 同口径)。仅策划阶段,不写代码符号。' },
