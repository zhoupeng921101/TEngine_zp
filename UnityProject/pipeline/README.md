...
流水线由 `/pipeline` 驱动:编排逻辑(boss)在 `.claude/skills/pipeline/SKILL.md`,三个执行角色是 `.claude/agents/` 下的 pipeline-plan / pipeline-dev / pipeline-test(角色卡即 agent 定义,spawn 自动注入)。自治模式脚本:`.claude/workflows/pipeline-auto.js`。
