...
> **开工** → 先读 `roles/<我的角色>.md` + `state/<我的角色>.md`(+ 相关 `../design-docs/`)
> **干活** → 按角色卡职责执行
> **收尾 / 每完成一个子步骤** → 立刻更新 `state/<我的角色>.md`
...
> 用 `subagents(action=list)` 核对在跑的子会话,再据各 `state/*.md` 交接区决定续跑。
> 编排动作(spawn/收完成/转棒/打回)发生后必须立刻更新 `state/boss.md`,尤其是子会话的 session key。

## 交接方式(方案 B:boss 编排)

由 main(boss)用 `sessions_spawn` 拉起角色 sub-agent,`sessions_yield` 等完成,再把上一棒产出喂给下一棒。
