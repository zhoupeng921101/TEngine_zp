...
- `run_tests` 带 `include_details` 的明细输出常超 token 上限被落盘到文件:用 python 读保存的 JSON,逐例 state 字段名是 `state`(值 `Passed`/`Failed`,不是 `status`/`result`/`resultState`),据此确认新增用例确实跑了(非被静默跳过)而非只看 summary 的 passed 总数(2026-06,player-info)
...
- Luban 直读 C3 类验收点取 GREEN 而非 BLOCKED 时,bytes 由真实生成表类(`GameConfig.<table>.Tb*`)经 `AssetDatabase.LoadAssetAtPath<TextAsset>` + `new Tb*(ByteBuf)` 解出且字段断言全中,即同时构成源 xlsx→bytes 一致性的最强交叉核验(字节布局正确、无错位),无须再手解 xlsx;C3 单独复跑确认 `state==Passed`(非 Inconclusive)即可锚定,不必逐字读 xlsx 单元格(2026-06,mail)
