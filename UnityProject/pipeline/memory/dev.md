...
- `RepairTemple` 式「已提交经济动作」一律成功后 `_undoStack.Clear()`(同交付),否则悔棋会倒回已扣币/已改状态。隔离测快照对该字段的回滚能力须手动改字段后 Undo(不能走会清栈的真实方法)(2026-06,piety-temple)
...
- Luban 字段 `##group` 列写多组要用逗号分隔的有效组名 `c,s`,不能写连写 `cs`:本工程 luban.conf 只定义了 c/s/e 三个单字符组,client 导出目标 groups=["c"]。写 `cs` 是一个 luban 不认识的组名,client 目标判不匹配→整列从生成的行类型里丢掉(范本 weightcfg.xlsx 全用 `c,s`)。症状:导表成功但生成的行类 .cs 缺字段,而表类 .cs 仍引用缺失的 `_v.Id` 等→编译失败。改 ##group 行重导即修。group=s/e 的字段不导出 client 目标,客户端生成的行类型不含它们→桥接 POCO 也不能含、helper 兜底别依赖它们(2026-06,numeric-system)
...
- 新增 Luban 表用 luban_helper.py `table add` 默认走「自动导入」(建 `num/#Num-xxx.xlsx` 子目录、注册 source=auto-import、丢掉 index/mode)。要传统的 `__tables__.xlsx` 注册 + 平铺 `num.xlsx` + 保 index=id/mode=map,必须加 `--no-auto-import`。且 `table add` 不会自动填 `read_schema_from_file` 列(留空),schema-in-file 的表须手动把该列设 True(与 item/weightcfg 对齐),否则 luban 可能找 XML 定义而非读表头(2026-06,numeric-system)
