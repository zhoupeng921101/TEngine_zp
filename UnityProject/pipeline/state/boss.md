...
tarot-blind-box 已于 2026-06-14 关单 PASS;自治授权随关单失效。
...
14. **[待用户定夺·可不做]** 盲盒持有计数(及 `MergeOrderState` 整体)当前不做跨会话磁盘存盘,只入悔棋快照(O3 默认)。如要盲盒跨会话保留,须给 `MergeOrderState` 加全量存盘(独立任务)。
