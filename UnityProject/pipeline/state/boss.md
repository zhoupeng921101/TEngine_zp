...
**xlsx 系统底层批次方向决策(2026-06-14 用户拍板)**:剩余队列(兑换码/排行榜/邮件)本质是服务器/在线功能,与本作离线·去变现·无服务器方向不合;用户选「**继续建底层,留服务器接缝**」——当可复用通用底层做,逻辑层可测、服务器侧抽接缝 + TODO,离线版 inert。兑换码已按此落地(2026-06-14 关单)。
**队列剩余**(续接同一决策,均「服务器底层 + 接缝」处理):排行榜(`1007排行榜底层.xlsx`)→ 邮件(`1002`,体量最大约 1.5MB)。TOAST(1008,纯 UI/阻塞于美术)与 音乐(1009,与 settings 音频重合)离线近空。
