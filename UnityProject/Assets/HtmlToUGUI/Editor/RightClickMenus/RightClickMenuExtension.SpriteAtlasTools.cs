...
{
    /// <summary>
    /// Project 窗口右键菜单扩展 — SpriteAtlas 转换工具集。
    /// 支持将 SpriteAtlas 导出为 TMP_SpriteAsset、Sprite(Multiple)、TextureSheet，以及拆分 Multiple 模式碎图。
    /// </summary>
    public partial class ProjectPanelRightClickExtension
    {
        ...
        static void SpriteAtlas2SpriteGirdSheetMenu()
        {
            ...
            foreach (var item in objs)
            {
                if (item is SpriteAtlas spAtlas)
                {
                    ...
                    string textureFileName = ResourceLoader.GetCombinePath(srcFileDir, srcFileNameWithoutExtension + "_girdsheet.png");
        ...
        /// </summary>
        /// <param name="sprites">要排列的精灵数组</param>
        /// <param name="outpathFileName">输出纹理文件路径</param>
        ...
        public static void SpriteAtlas2SpriteSheet(SpriteAtlas atlas)
        {
            ...
            string srcFileDir = Path.GetDirectoryName(srcFileName);
            string srcFileNameWithoutExtension = Path.GetFileNameWithoutExtension(srcFileName);
            string textureFileName = ResourceLoader.GetCombinePath(srcFileDir, srcFileNameWithoutExtension + "_sheet.png");
        ...
        /// <summary>
        /// 将 SpriteAtlas 导出为 TextMeshPro SpriteAsset（.asset）。
        /// </summary>
        /// <param name="atlas">要导出的 SpriteAtlas</param>
        public static void SpriteAtlas2TmpSprite(SpriteAtlas atlas)
        {
            string srcFileName = AssetDatabase.GetAssetPath(atlas);
            string srcFileDir = Path.GetDirectoryName(srcFileName);
            string srcFileNameWithoutExtension = Path.GetFileNameWithoutExtension(srcFileName);
            string tmpSpriteAssetName = ResourceLoader.GetCombinePath(srcFileDir, srcFileNameWithoutExtension + ".asset");
            string textureFileName = ResourceLoader.GetCombinePath(srcFileDir, srcFileNameWithoutExtension + ".png");
        ...
        static void ExportSpriteMultiple()
        {
            ...
            for (int i = 0; i < selectAssetsCount; i++)
            {
                ...
                var outputDir = ResourceLoader.GetCombinePath(Path.GetDirectoryName(spFileName), $"{Path.GetFileNameWithoutExtension(spFileName)}_sliced");
                ...
                for (int spIndex = 0; spIndex < childrenSpCount; spIndex++)
                {
                    ...
                    tex.Apply();
                    string fileName = ResourceLoader.GetCombinePath(outputDir, $"{spDt.name}.png");
