...
namespace GameLogic.BlockBlast.Tests.Algorithms
{
    ...
    {
        /// <summary>固定 seed 的可移植随机源,让算法结果可重现(去单例化后随机源显式传入)。</summary>
        private IRandomSource _rng;

        [SetUp]
        public void SetUp()
        {
            _rng = new XorShift128PlusRng(12345);
        }
        ...
        public void UniformRandomShape_ReturnsValidCommonId()
        {
            for (int i = 0; i < 20; i++)
            {
                int id = BlockAlgorithms.UniformRandomShape(_rng);
        ...
        {
            var trio = BlockAlgorithms.SampleTrio(_rng);
        ...
        public void GenerateTrio_AllAlgorithms_ReturnValidTrio(AlgorithmKind algo)
        {
            ...
            var trio = BlockAlgorithms.GenerateTrio(algo, board, _rng);
        ...
        public void FallbackTrio_AlmostFullBoard_ReturnsAllOnes()
        {
            ...
            var trio = BlockAlgorithms.FallbackTrio(b, _rng);
            Assert.AreEqual(3, trio.Length);
            // 兜底必然是 [1,1,1]
        ...
        public void BoardClearGreedyTrio_NonEmptyBoard_ReturnsThreeIds()
        {
            ...
            var trio = BlockAlgorithms.BoardClearGreedyTrio(b, _rng);
            Assert.AreEqual(3, trio.Length);
        }
    }
}
