...
namespace PSDUIImporter
{
    public class PSDImportCtrl
    {
        ...
        public void BeginDrawUILayers()
        {
            ...
            SetTouchClose(obj); // 设置自动关闭的点击屏蔽
            GameObject go = SetContainer(obj); //把UI装进容器中方便管理动画
            for (int layerIndex = 0; layerIndex < psdUI.layers.Length; layerIndex++)
            {
                DrawLayer(psdUI.layers[layerIndex], go);
            }
            AssetDatabase.Refresh();
        }
        private void SetTouchClose(RectTransform parent)
        {
            GameObject tmp = new GameObject("TouchAutoClose");
            tmp.AddComponent<UnityEngine.UI.Empty4Raycast>();
            tmp.AddComponent<UnityEngine.UI.Button>();
            UnityGameFramework.Runtime.MVVM.DataCommand bind = tmp.GetOrAddComponent<UnityGameFramework.Runtime.MVVM.DataCommand>();
            bind.Command = "TouchAutoClose";
            bind.Type = EventTriggerType.PointerClick;
            RectTransform rect = tmp.GetOrAddComponent<RectTransform>();
            rect.SetParent(parent);
            SetRect(rect);
        }
        private GameObject SetContainer(RectTransform parent)
        {
            GameObject tmp = new GameObject("container");
            RectTransform containerRT = tmp.GetOrAddComponent<RectTransform>();
            UnityGameFramework.Runtime.MVVM.DataBinding bind = tmp.GetOrAddComponent<UnityGameFramework.Runtime.MVVM.DataBinding>();
            bind.Parameters = new string[2] { "0", "OnAniComplete" };
            containerRT.SetParent(parent);
            SetRect(containerRT);
            return containerRT.gameObject;
        }
        private void SetRect(RectTransform rect)
        {
        ...
        private void ImportLayer(Layer layer, string baseDirectory)
        {
            if (layer.image != null)
            {
                ...
                {
                    string texturePathName = image.getPath();
                    ...
                    if (textureImporter != null && image.imageType != ImageType.Texture) //Texture类型不设置属性
                    {
                        ...
                        textureImporter.SetPlatformTextureSettings("iPhone", textureImporter.maxTextureSize, TextureImporterFormat.ASTC_RGBA_5x5);
        ...
        {
            // Debug.LogWarning (args[1]); 左上右下 0123
            // borderLeft borderBottom borderRight borderTop 左 下 右 上  x,y,z,w 0321
            // Debug.LogWarning (args[0] + " " + args[1] + " " + args[2] + " " + args[3]);
