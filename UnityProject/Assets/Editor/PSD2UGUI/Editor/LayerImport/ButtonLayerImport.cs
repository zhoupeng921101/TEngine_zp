        ...
        public void DrawLayer(Layer layer, GameObject parent)
        {
            ...
            if (layer.layers != null)
            {
                for (int imageIndex = 0; imageIndex < layer.layers.Length; imageIndex++)
                {
                    ...
                    {
                        if (image.imageSource == ImageSource.Custom || image.imageSource == ImageSource.Common || image.imageSource == ImageSource.Global)
                        {
                            string assetPath = image.getPath();
                            ...
                            {
                                UnityEngine.UI.Image btnImg = button.transform.Find("Image").GetComponent<UnityEngine.UI.Image>();
                                btnImg.transform.SetParent(button.transform.parent);
                                btnImg.sprite = sprite;
                                btnImg.gameObject.SetActive(true);
                                btnImg.rectTransform.sizeDelta = new Vector2(image.size.width, image.size.height);
                                btnImg.rectTransform.anchoredPosition = new Vector2(image.position.x, image.position.y);
                                btnImg.transform.SetParent(button.transform);
