...
namespace GameLogic
{
    ...
    /// </summary>
    /// <remarks>
    /// 全代码构建（沿 <see cref="MainMenuWindow"/> / <c>RankWindow</c> 全代码生成范式，prefab 仅为空壳承载脚本，无 Inspector 拖线）。
    ...
    /// </remarks>
    [Window(UILayer.UI, location: "BackpackWindow", fullScreen: false)]
    public sealed class BackpackWindow : UIWindowMono
    {
        ...
        private async UniTaskVoid UseItemAsync(int itemId)
        {
            ...
            {
                Log.Warning($"[BackpackWindow] 使用道具异常：{e.Message}");
            }
            finally
            {
                _useInFlight = false;
        ...
        private void OnClose() => GameModule.UI.CloseUI<BackpackWindow>();
    }
}
