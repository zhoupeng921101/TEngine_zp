...
namespace GameLogic
{
    ...
    /// 动态格子按服务端库存投影在运行时建入 <c>m_rect_Grid</c> 容器（格子数量与内容数据驱动，不入 prefab）。
    ...
    public sealed partial class UIBagPanel : UIPanelMono
    {
        ...
        private void RefreshGrid()
        {
            ...
            for (int i = m_rect_Grid.childCount - 1; i >= 0; i--)
            {
                Object.Destroy(m_rect_Grid.GetChild(i).gameObject);
            }
        ...
        private void BuildHoldingCell(int index, int itemId, int count)
        {
            ...
            // 数量角标（右下）。
            UGuiFactory.CreateText(m_rect_Grid, "Count",
        ...
        private void BuildLotCell(int index, InventoryLot lot)
        {
            ...
            var cd = UGuiFactory.CreateText(m_rect_Grid, "Countdown",
        ...
        {
            var icon = UGuiFactory.CreateImage(m_rect_Grid, "Icon", center.x, center.y,
                CellSize * 0.62f, CellSize * 0.62f, iconColor);
            icon.raycastTarget = false;
        }

        /// <summary>
        /// 格子底板 Button（点击→使用）建于 <see cref="m_rect_Grid"/>；图标 / 数量 / 倒计时同建于 m_rect_Grid 同坐标系、叠在其上。
        /// 返回图标品质色（查道具表 <see cref="RewardDisplay.QualityColor"/>，查无退化白）。
        /// </summary>
        private Color BuildCellShell(int index, int itemId)
        {
            var center = CellCenter(index);
            var btn = UGuiFactory.CreateButton(m_rect_Grid, "Cell_" + index, center.x, center.y, CellSize, CellSize,
