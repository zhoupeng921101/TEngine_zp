    ...
    public class LoopListView : MonoBehaviour, IBeginDragHandler, IEndDragHandler, IDragHandler
    {
        ...
        */
            ...
            LoopListViewInitParam initParam = null)
        {
            ...
            if (_listViewInited)
            {
                Log.Error("LoopListView.InitListView method can be called only once.");
            }
...
#if UNITY_EDITOR
            GameObject go = GameModule.Resource.LoadAsset<GameObject>(resPath);
#else
            GameObject go = null;
            Log.Error($"[LoopListView] 运行时禁用同步资源加载，按名加载 item 预制 '{resPath}' 失败。请改用 GameObject 重载传入预载好的预制。");
#endif
            if (go != null)
            {
                go.SetActive(false);
        ...
        */
        ...
        void InitItemPool()
        {
            foreach (ItemPrefabConfData data in itemPrefabDataList)
            {
                ...
                {
                    Log.Error("A item prefab with name " + prefabName + " has existed!");
                    continue;
                }
                RectTransform rtf = data.mItemPrefab.GetComponent<RectTransform>();
                if (rtf == null)
                {
                    Log.Error("RectTransform component is not found in the prefab " + prefabName);
                    continue;
                }
                AdjustAnchor(rtf);
                AdjustPivot(rtf);
            ...
            float distanceForNew1)
        {
            ...
            if (_isVertList)
            {
                ...
                while (needContinueCheck)
                {
                    ...
                    {
                        Log.Error("UpdateListView Vertical while loop " + checkCount +
            ...
            else
            {
                ...
                while (needContinueCheck)
                {
                    ...
                    if (checkCount >= maxCount)
                    {
                        Log.Error("UpdateListView  Horizontal while loop " + checkCount +
                                       " times! something is wrong!");
                        break;
                    }
