...
namespace GameLogic.BlockBlast.Player
{
    ...
    /// 登录快照 + 整批刷新推送是 normal 订单唯一来源,交付走 RPC、奖励由服务端发 + delta-push 回(已有 MetaCurrencySync 落地)。
    ///
    ...
    /// </summary>
    /// <remarks>
    /// 【双计避免】normal 订单交付的 Energy/Piety 改由服务端 RPC 发放 + <c>G2C_PropertyDeltaPush</c> 推回。客户端交付路径
    /// (<see cref="MergeOrderState.Deliver"/> 服务端权威分支)<b>不再本地实发</b> Energy/Piety,故 <c>MetaCurrencySync.ReportPending</c>
    /// 的基线 diff 看不到交付增量、不会重复上报(无需额外排除累加器:不本地加 = 无 delta)。
    ///
    ...
    public sealed class OrderSync
    {
        ...
        public OrderSync(IOrderRpcGateway gateway)
        {
        ...
        /// Success → 应用回带快照校正槽位(奖励经 delta-push 回);其它码 → 退还库存 + 回滚计数(失败不丢库存)。
        ...
        public async UniTask RequestDeliver(MergeOrderState state, int slot, Order consumed)
        {
            ...
            {
                // 成功:服务端权威快照(含已置空本槽 + 可能的整批刷新)覆盖本地槽位;奖励走 delta-push,不在此发。
